...
try {
	...
	if (exec('which composer | wc -l') > 0) {
		...
		if (version_compare(PHP_VERSION, '8.0.0') >= 0) {
			echo "Update composer file for PHP8+ ...\n";
			shell_exec('cd ' . __DIR__ . '/../;export COMPOSER_ALLOW_SUPERUSER=1;export COMPOSER_HOME="/tmp/composer";'
			  .system::getCmdSudo().' composer update --no-progress --no-interaction --no-dev > /dev/null 2>&1');
		} else {
			echo "Install composer file for PHP7.x ...\n";
			shell_exec('cd ' . __DIR__ . '/../;export COMPOSER_ALLOW_SUPERUSER=1;export COMPOSER_HOME="/tmp/composer";'
		...
		echo "WARN: No composer installation available!\n";
	}
	echo "[PROGRESS][58]\n";
