...
if (!isset($_GET['step'])) {
	...
	$output = array();
	exec('php ' . dirname(__FILE__) . '/update.php step=10', $output, $rc);
	testRc($rc, $output, 10);
	echo "Launch step 20 : update updater";
	$output = array();
	exec('php ' . dirname(__FILE__) . '/update.php step=20', $output, $rc);
	testRc($rc, $output, 20);
	echo "Launch step 30 : update jeedom file and database";
	$output = array();
	exec('php ' . dirname(__FILE__) . '/update.php step=30', $output, $rc);
	testRc($rc, $output, 30);
	echo "Launch step 40 : update jeedom";
	$output = array();
	exec('php ' . dirname(__FILE__) . '/update.php step=40', $output, $rc);
	testRc($rc, $output, 40);
} else {
	if (function_exists('step' . $_GET['step'])) {
		'step' . $_GET['step']();
	}
}
