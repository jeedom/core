	...
	public function additionalData() {
		...
		if ($this->getLink_type() == 'eqLogic') {
			...
			if ($this->getConfiguration('3d::widget') == 'door') {
				...
				$return['color'] = $this->getConfiguration('3d::widget::door::windowopen');
				...
				if (is_object($cmd) && $cmd->getType() == 'info') {
					...
					if ($cmd_value) {
						$return['color'] = $this->getConfiguration('3d::widget::door::windowclose');
					}
				}
				if ($return['color'] != $this->getConfiguration('3d::widget::door::windowopen')) {
					$cmd = cmd::byId(str_replace('#', '', $this->getConfiguration('3d::widget::door::shutter')));
					if (is_object($cmd) && $cmd->getType() == 'info') {
						if ($cmd->execCmd()) {
							$return['color'] = $this->getConfiguration('3d::widget::door::shutterclose');
						}
					}
				}
			}
