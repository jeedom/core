			...
			$sql = 'DELETE FROM scenarioSubElement WHERE id NOT IN (-1';
				...
				*/
				...
				public static function timelineDisplay($_event) {
					...
					$return['html'] .= ' <span class="label-sm label-info" title="{{Scénario déclenché par}}">' . $_event['trigger'] . '</span>';
					$return['html'] .= ' <i class="fas fa-file-alt pull-right cursor bt_scenarioLog" title="{{Log du scénario}}"></i> ';
					$return['html'] .= ' <i class="fas fa-share pull-right cursor bt_gotoScenario" title="{{Aller au scénario}}"></i> ';
					$return['html'] .= '</div>';
					$return['html'] .= '</div>';
					return $return;
				}
