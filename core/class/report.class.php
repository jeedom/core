...
class report {
	...
	public static function generate($_url, $_type, $_name, $_format = 'png', $_parameter = array()) {
		...
		if (shell_exec('which chromium | wc -l') > 0) {
			...
			if (isset($_parameter['tab']) && $_parameter['tab'] != '') {
				$_url .= '#' . $_parameter['tab'];
			}
			if ($_format == 'pdf') {
				$cmd = 'chromium --headless --no-sandbox --disable-gpu --no-pdf-header-footer --no-pdf-header-footer --print-to-pdf=' . $out . ' --window-size=' . $min_width . ',' . $min_height . ' "' . $_url . '"';
			} else {
