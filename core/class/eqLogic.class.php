...
class eqLogic {
	...
	public static function byObjectId($_object_id, $_onlyEnable=true, $_onlyVisible=false, $_eqType_name=null, $_logicalId=null, $_orderByName=false, $_onlyHasCmds=false) {
		...
		if ($_onlyHasCmds) {
			$sqlAdd = ' AND id IN (SELECT eqLogic_id FROM cmd)';
			if ($_onlyHasCmds == 'info') $sqlAdd = ' AND id IN (SELECT eqLogic_id FROM cmd WHERE type="info")';
			if ($_onlyHasCmds == 'action') $sqlAdd = ' AND id IN (SELECT eqLogic_id FROM cmd WHERE type="action")';
			$sql .= $sqlAdd;
		}
		if ($_orderByName) {
			$sql .= ' ORDER BY `name`';
