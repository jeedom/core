...
class eqLogic {
	...
	public function batteryWidget($_version = 'dashboard') {
		...
		$batterySince = 'NA';
		if ($batteryTime != 'NA') {
			$batterySince = round(((strtotime(date("Y-m-d")) - strtotime(date("Y-m-d", strtotime($batteryTime)))) / 86400));
		}
		...
		$html .= '<div style="text-align : center; cursor:default;">' . __('Le', __FILE__) . ' ' . date("Y-m-d H:i:s", strtotime($this->getStatus('batteryDatetime', __('inconnue', __FILE__)))) . '</div>';
