...
class repo_market {
	...
	public static function backup_restore($_backup) {
		...
		com_shell::execute('sudo chmod 777 -R /tmp/jeedom_gnupg');
		$cmd = 'cd '.$backup_dir.';wget https://'.config::byKey('market::username') . ':' . config::byKey('market::password').'@' .str_replace('https://','',config::byKey('service::backup::url')) . '/webdav/'. config::byKey('market::username').'/'.  urlencode(config::byKey('market::cloud::backup::name')).'/'.$_backup;
		com_shell::execute($cmd);
