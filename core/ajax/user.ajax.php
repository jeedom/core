		...
		foreach ($users as $user_json) {
			...
			if (!is_object($user)) {
				...
				if (isset($user_json['password']) && $user_json['password'] == '') {
					unset($user_json['password']);
				}
				if (isset($user_json['hash']) && $user_json['hash'] != '') {
					unset($user_json['hash']); // hash cannot be changed to a chosen value, it is generated by the system; to regenerate a new hash, it must be set to empty string, which will generate a new hash
				}
				if ($user->getId() == $_SESSION['user']->getId()) {
					if (isset($user_json['enable']) && $user_json['enable'] == 0) {
						throw new Exception(__('Vous ne pouvez pas désactiver le compte avec lequel vous êtes connecté', __FILE__));
					}
