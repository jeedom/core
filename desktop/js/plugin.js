...
$(".pluginDisplayCard").off('click').on('click', function (event) {
  if (event.ctrlKey) {
    var url = '/index.php?v=d&p=plugin&id='+$(this).attr('data-plugin_id')
...
$('.pluginDisplayCard').off('mouseup').on('mouseup', function (event) {
  if( event.which == 2 ) {
    event.preventDefault()
    var id = $(this).attr('data-plugin_id')
    $('.pluginDisplayCard[data-plugin_id="'+id+'"]').trigger(jQuery.Event('click', { ctrlKey: true }))
  }
})
