							...
							try {
								...
								foreach ($plugins as $plugin) {
									...
									foreach ($specialAttributes['user'] as $key => $config) {
										...
										echo '<label class="col-lg-2 col-sm-4 col-xs-2 control-label">'.$config['name'][translate::getLanguage()].'</label>';
										echo '<div class="col-lg-2 col-sm-4 col-xs-3">';
										switch ($config['type']) {
											case 'input':
											echo '<input class="form-control userAttr" data-l1key="options" data-l2key="plugin::'.$plugin->getId().'::'.$key.'"/>';
											break;
											case 'number':
											echo '<input type="number" class="form-control userAttr" data-l1key="options" data-l2key="plugin::'.$plugin->getId().'::'.$key.'" min="'.(isset($config['min']) ? $config['min'] : '').'" max="'.(isset($config['max']) ? $config['max'] : '').'" />';
											break;
											case 'select':
											echo '<select class="form-control userAttr" data-l1key="options" data-l2key="plugin::'.$plugin->getId().'::'.$key.'">';
