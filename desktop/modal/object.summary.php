...
<table class="table table-bordered table-condensed tablesorter" id="table_ObjectSummary">
	<thead>
		<tr>
			...
			<th data-sorter="false" data-filter="false">{{Résumé Défini}} <sup title="Si grisé, alors il n'est pas remonté en résumé global">?</th>
	...
	<tbody>
		<?php
			...
			foreach ($allObject as $object) {
				echo '<tr class="tr_object" data-object_id="' . $object->getId() . '"><td><span class="label label-info">' . $object->getId() . '</span></td>';
				echo '<td>';
				...
				if ($father) {
					echo '<td><span>' . $father->getHumanName(true, true) . '</span></td>';
				} else {
					echo '<td><span class="label label-info"></span></td>';
				}
				if ($object->getIsVisible()) {
					echo '<td><span class="label label-success" title="{{Oui}}"><i class="fas fa-check"></i></span></td>';
				} else {
					echo '<td><span class="label label-danger" title="{{Non}}"><i class="fas fa-times"></i></span></td>';
				}
