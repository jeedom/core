...
<div id="md_dbCheck" data-modalType="md_dbCheck">
...
<script>
  document.getElementById('md_dbCheck').addEventListener('click', function(event) {
    var _target = null
    if (_target = event.target.closest('.bt_correctTable')) {
      if (_target.dataset.package == 'all') {
        var text = '{{Êtes-vous sûr de vouloir corriger toutes les tables ?}}'
      } else {
        var text = '{{Êtes-vous sûr de vouloir corriger la table}} ' + _target.dataset.table + ' ?'
      }
      jeeDialog.confirm({
        title: '{{Correction de la base de données}}',
        message: text
      }, function(result) {
        if (result) {
