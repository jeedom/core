...
<div id="div_displayEqLogicConfigure">
  ...
  <div class="input-group pull-right" style="display:inline-flex">
    <span class="input-group-btn">
      <a id="bt_eqLogicConfigExpert" class="btn btn-success btn-sm roundedLeft"><i class="fas fa-flask"></i> {{Mode avancé}}
      </a><a id="bt_eqLogicConfigureSave" class="btn btn-warning btn-sm roundedRight"><i class="fas fa-check-circle"></i> {{Sauvegarder}}</a>
    </span>
  </div>
   -->

  <div class="row">
    <a id="bt_eqLogicConfigureSave" class="btn btn-warning btn-sm pull-right"><i class="fas fa-check-circle"></i> {{Sauvegarder}}</a>
    <div class="col-sm-12" >
              ...
              <div id="div_eqLogicCmds">
                <?php
                  ...
                  foreach (($eqLogic->getCmd()) as $cmd) {
                    ...
                    $display .= '<div class="cmdConfig" style="padding: 2px;" data-id="' . $cmd->getId() . '">';
                    $display .= '<a class="btn btn-default btn-xs cursor bt_cmdConfig" data-toggle="collapse" data-target="#cmdConfig' . $cmd->getId() . '">'.$cmd->getName().' ('.$cmd->getType().' | '.$cmd->getSubType().')</a>';
                      ...
                      $display .= '<div id="cmdConfig' . $cmd->getId() . '" class="collapse" style="margin-top: 8px;">';
                        $display .= '<table class="table table-bordered table-condensed">';
                          ...
                          $display .= '<td><input type="checkbox" class="'.$thisclassAttrib.'" data-l1key="display" data-l2key="showNameOndashboard"></td></tr>';
                          ...
                          $display .= '<td><input type="checkbox" class="'.$thisclassAttrib.'" data-l1key="display" data-l2key="showStatsOndashboard"></td></tr>';
...
function setModal() {
  $('#md_modal').dialog("option","width",'auto')
  ...
  if (datas && datas.width && datas.height && datas.top && datas.left) {
    ...
    $('#md_modal').height(datas.height-34)
...
$('#bt_eqLogicConfigExpert').off().on('click', function(event) {
  var eqId = eqLogicInfo.id
  $('#md_modal').dialog({title: "{{Configuration}}"}).load('index.php?v=d&modal=eqLogic.configure&eqLogic_id='+eqId).dialog('open')
})

$('#bt_eqLogicConfigureSave').off().on('click', function(event) {
