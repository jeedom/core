	...
	header("Last-Modified: " . gmdate("D, d M Y H:i:s") . " GMT");if ($_GET["file"] == "favicon.ico") {
		...
		header("Content-Type: image/gif");switch ($_GET["file"]) {
			case "plus.gif":echo "GIF89a\0\0\0001îîî\0\0€™™™\0\0\0!ù\0\0\0,\0\0\0\0\0\0!„©ËíMñÌ*)¾oú¯) q•¡eˆµî#ÄòLË\0;";
				...
				break;case "down.gif":echo "GIF89a\0\0\0001îîî\0\0€™™™\0\0\0!ù\0\0\0,\0\0\0\0\0\0 „©ËíMñÌ*)¾[Wþ\\¢ÇL&ÙœÆ¶•\0Çò\0;";
...
get_vals($I, $c = 0) {
	global $e;
	$K = array();
	$J = $e->query($I);if (is_object($J)) {
		while ($L = $J->fetch_row()) {
			$K[] = $L[$c];
		}
	}
	return $K;}function
get_key_vals($I, $f = null, $cg = 0) {
	global $e;if (!is_object($f)) {
		$f = $e;
	}
...
unique_array($L, $w) {
	foreach ($w as $v) {
		if (preg_match("~PRIMARY|UNIQUE~", $v["type"])) {
			$K = array();foreach ($v["columns"] as $z) {
				if (!isset($L[$z])) {
					continue
						2;
				}
...
where($Z, $m = array()) {
	...
	$K = array();foreach ((array) $Z["where"] as $z => $X) {
		...
		$c = escape_key($z);
		$K[] = $c . (($y == "sql" && preg_match('~^[0-9]*\\.[0-9]*$~', $X)) || $y == "mssql" ? " LIKE " . q(addcslashes($X, "%_\\")) : " = " . unconvert_field($m[$z], q($X)));if ($y == "sql" && preg_match('~char|text~', $m[$z]["type"]) && preg_match("~[^ -@]~", $X)) {
...
query_redirect($I, $B, $D, $Ue = true, $cc = true, $ic = false, $bg = "") {
	...
	return
		true;}function
queries($I) {
	global $e;static $Pe = array();static $Cf;if (!$Cf) {
		$Cf = microtime(true);
	}
	if ($I === null) {
		return
...
queries_redirect($B, $D, $Ue) {
	list($Pe, $bg) = queries(null);return
...
get_file($z, $ub = false) {
	...
	$K = '';foreach ($mc["error"] as $z => $k) {
		...
		$F = $mc["name"][$z];
		$jg = $mc["tmp_name"][$z];
		$fb = file_get_contents($ub && preg_match('~\\.gz$~', $F) ? "compress.zlib://$jg" : $jg);if ($ub) {
			$Cf = substr($fb, 0, 3);if (function_exists("iconv") && preg_match("~^\xFE\xFF|^\xFF\xFE~", $Cf, $af)) {
...
column_foreign_keys($Q) {
	global $b;
	$K = array();foreach ($b->foreignKeys($Q) as $n) {
		foreach ($n["source"] as $X) {
			$K[$X][] = $n;
		}
	}
	return $K;}function
enum_input($U, $wa, $l, $Y, $Sb = null) {
	global $b;
	preg_match_all("~'((?:[^']|'')*)'~", $l["length"], $rd);
	$K = ($Sb !== null ? "<label><input type='$U'$wa value='$Sb'" . ((is_array($Y) ? in_array($Sb, $Y) : $Y === 0) ? " checked" : "") . "><i>" . 'empty' . "</i></label>" : "");foreach ($rd[1] as $t => $X) {
...
input($l, $Y, $q) {
	global $e, $tg, $b, $y;
	$F = h(bracket_escape($l["field"]));
	echo "<td class='function'>";if (is_array($Y) && !$q) {
		...
		$q = "json";}
	$cf = ($y == "mssql" && $l["auto_increment"]);if ($cf && !$_POST["save"]) {
		$q = null;
	}

	$xc = (isset($_GET["select"]) || $cf ? array("orig" => 'original') : array()) + $b->editFunctions($l);
	$wa = " name='fields[$F]'";if ($l["type"] == "enum") {
		echo
		...
		$pc = 0;foreach ($xc as $z => $X) {
			...
			$pc++;}
		$Wd = ($pc ? " onchange=\"var f = this.form['function[" . h(js_escape(bracket_escape($l["field"]))) . "]']; if ($pc > f.selectedIndex) f.selectedIndex = $pc;\" onkeyup='keyupChange.call(this);'" : "");
		$wa .= $Wd;
		$Cc = (in_array($q, $xc) || isset($xc[$q]));
		echo (count($xc) > 1 ? "<select name='function[$F]' onchange='functionChange(this);'" . on_help("getTarget(event).value.replace(/^SQL\$/, '')", 1) . ">" . optionlist($xc, $q === null || $Cc ? $q : "") . "</select>" : nbsp(reset($xc))) . '<td>';
		$Pc = $b->editInput($_GET["edit"], $l, $wa, $Y);if ($Pc != "") {
			...
			echo "<textarea$wa>" . h($Y) . '</textarea>';} elseif ($q == "json") {
			echo "<textarea$wa cols='50' rows='12' class='jush-js'>" . h($Y) . '</textarea>';
		} else {
			$yd = (!preg_match('~int~', $l["type"]) && preg_match('~^(\\d+)(,(\\d+))?$~', $l["length"], $C) ? ((preg_match("~binary~", $l["type"]) ? 2 : 1) * $C[1] + ($C[3] ? 1 : 0) + ($C[2] && !$l["unsigned"] ? 1 : 0)) : ($tg[$l["type"]] ? $tg[$l["type"]] + ($l["unsigned"] ? 0 : 1) : 0));if ($y == 'sql' && $e->server_info >= 5.6 && preg_match('~time~', $l["type"])) {
				$yd += 7;
			}
...
process_input($l) {
	global $b;
	$Jc = bracket_escape($l["field"]);
	$q = $_POST["function"][$Jc];
	$Y = $_POST["fields"][$Jc];if ($l["type"] == "enum") {
		if ($Y == -1) {
			return
				false;
		}
	...
	if ($q == "json") {
		...
		return $Y;}if (preg_match('~blob|bytea|raw|file~', $l["type"]) && ini_bool("file_uploads")) {
		$mc = get_file("fields-$Jc");if (!is_string($mc)) {
			return
				false;
		}
		return
		q($mc);}
	return $b->processInput($l, $Y, $q);}function
fields_from_edit() {
	global $j;
	...
	foreach ((array) $_POST["fields"] as $z => $X) {
		...
		$K[$F] = array("field" => $F, "privileges" => array("insert" => 1, "update" => 1), "null" => 1, "auto_increment" => ($z == $j->primary));}
	return $K;}function
search_tables() {
	global $b, $e;
	...
	$uc = false;foreach (table_status('', true) as $Q => $R) {
		$F = $b->tableName($R);if (isset($R["Engine"]) && $F != "" && (!$_POST["tables"] || in_array($Q, $_POST["tables"]))) {
			$J = $e->query("SELECT" . limit("1 FROM " . table($Q), " WHERE " . implode(" AND ", $b->selectSearchProcess(fields($Q), array())), 1));if (!$J || $J->fetch_row()) {
				if (!$uc) {
					echo "<ul>\n";
					$uc = true;}
...
dump_headers($Ic, $Ed = false) {
	global $b;
	$K = $b->dumpHeaders($Ic, $Ed);
	$me = $_POST["output"];if ($me != "text") {
...
count_rows($Q, $Z, $Vc, $s) {
	global $y;
	$I = " FROM " . table($Q) . ($Z ? " WHERE " . implode(" AND ", $Z) : "");return ($Vc && ($y == "sql" || count($s) == 1) ? "SELECT COUNT(DISTINCT " . implode(", ", $s) . ")$I" : "SELECT COUNT(*)" . ($Vc ? " FROM (SELECT 1$I$zc) x" : $I));}function
slow_query($I) {
	global $b, $T;
	$i = $b->database();
	$cg = $b->queryTimeout();if (support("kill") && is_object($f = connect()) && ($i == "" || $f->select_db($i))) {
		$bd = $f->result("SELECT CONNECTION_ID()");
...
edit_form($a, $m, $L, $Ag) {
	global $b, $y, $T, $k;
	$Pf = $b->tableName(table_status1($a, true));
	page_header(($Ag ? 'Edit' : 'Insert'), $k, array("select" => array($a, $Pf)), $Pf);if ($L === false) {
...
';if (!$m) {
		...
		echo "<table cellspacing='0' onkeydown='return editingKeydown(event);'>\n";foreach ($m as $F => $l) {
			...
			$vb = $_GET["set"][bracket_escape($F)];if ($vb === null) {
				$vb = $l["default"];if ($l["type"] == "bit" && preg_match("~^b'([01]*)'\$~", $vb, $af)) {
					$vb = $af[1];
				}
			}
			...
			$q = ($_POST["save"] ? (string) $_POST["function"][$F] : ($Ag && $l["on_update"] == "CURRENT_TIMESTAMP" ? "now" : ($Y === false ? null : ($Y !== null ? '' : 'NULL'))));if (preg_match("~time~", $l["type"]) && $Y == "CURRENT_TIMESTAMP") {
				$Y = "";
				$q = "now";}
			input($l, $Y, $q);
			echo "\n";}if (!support("table")) {
...
session_cache_limiter("");if (!defined("SID")) {
	...
	$pe = array(0, preg_replace('~\\?.*~', '', $_SERVER["REQUEST_URI"]), "", $ba);if (version_compare(PHP_VERSION, '5.2.0') >= 0) {
		$pe[] = true;
	}
...
lang($ng, $Md = null) {
	...
	sprintf($ng, $Md);}if (extension_loaded('pdo')) {
	class
	Min_PDO extends
	PDO {
		...
		__construct() {
			global $b;
			$Be = array_search("SQL", $b->operators);if ($Be !== false) {
				unset($b->operators[$Be]);
			}
		}function
		dsn($Ib, $V, $we) {
			try {parent::__construct($Ib, $V, $we);} catch (Exception $ac) {auth_error($ac->getMessage());}$this->setAttribute(13, array('Min_PDOStatement'));
...
Min_SQL {
	...
	select($Q, $N, $Z, $s, $de = array(), $_ = 1, $G = 0, $Ie = false) {
		global $b, $y;
		$Vc = (count($s) < count($N));
		$I = $b->selectQueryBuild($N, $Z, $s, $de, $_, $G);if (!$I) {
	...
	define("DRIVER", "server");if (extension_loaded("mysqli")) {
		...
		MySQLi {
			...
			connect($O = "", $V = "", $we = "", $qb = null, $Ae = null, $zf = null) {
				mysqli_report(MYSQLI_REPORT_OFF);list($Gc, $Ae) = explode(":", $O, 2);
				$K = @$this->real_connect(($O != "" ? $Gc : ini_get("mysqli.default_host")), ($O . $V != "" ? $V : ini_get("mysqli.default_user")), ($O . $V . $we != "" ? $we : ini_get("mysqli.default_pw")), $qb, (is_numeric($Ae) ? $Ae : ini_get("mysqli.default_port")), (!is_numeric($Ae) ? $Ae : $zf));return $K;}function
			set_charset($La) {
			...
			quote($Gf) {return "'" . $this->escape_string($Gf) . "'";}}} elseif (extension_loaded("mysql") && !(ini_get("sql.safe_mode") && extension_loaded("pdo_mysql"))) {
		class
		Min_DB {
			var $extension = "MySQL", $server_info, $affected_rows, $errno, $error, $_link, $_result;function
			connect($O, $V, $we) {
				$this->_link = @mysql_connect(($O != "" ? $O : ini_get("mysql.default_host")), ("$O$V" != "" ? $V : ini_get("mysql.default_user")), ("$O$V$we" != "" ? $we : ini_get("mysql.default_password")), true, 131072);if ($this->_link) {
		...
		Min_PDO {
			...
			connect($O, $V, $we) {
				$this->dsn("mysql:charset=utf8;host=" . str_replace(":", ";unix_socket=", preg_replace('~:(\\d)~', ';port=\\1', $O)), $V, $we);return
					true;}function
			set_charset($La) {$this->query("SET NAMES $La");}function
			select_db($qb) {return $this->query("USE " . idf_escape($qb));}function
			query($I, $ug = false) {
				$this->setAttribute(1000, !$ug);return
				parent::query($I, $ug);}}}
	class
	Min_Driver extends
	Min_SQL {
		...
		insertUpdate($Q, $M, $He) {
			...
			$Fe = "INSERT INTO " . table($Q) . " (" . implode(", ", $d) . ") VALUES\n";
			$Hg = array();foreach ($d as $z) {
				$Hg[$z] = "$z = VALUES($z)";
			}
	...
	connect() {
		...
		$e = new
			Min_DB;
		$mb = $b->credentials();if ($e->connect($mb[0], $mb[1], $mb[2])) {
			$e->set_charset(charset($e));
			$e->query("SET sql_quote_show_create = 1, autocommit = 1");return $e;}
		$K = $e->error;if (function_exists('iconv') && !is_utf8($K) && strlen($lf = iconv("windows-1250", "utf-8", $K)) > strlen($K)) {
			$K = $lf;
		}
		return $K;}function
	get_databases($qc) {
	...
	db_collation($i, $Xa) {
		global $e;
		$K = null;
		$g = $e->result("SHOW CREATE DATABASE " . idf_escape($i), 1);if (preg_match('~ COLLATE ([^ ]+)~', $g, $C)) {
			$K = $C[1];
	...
	count_tables($h) {
		$K = array();foreach ($h as $i) {
			$K[$i] = count(get_vals("SHOW TABLES IN " . idf_escape($i)));
		}
		return $K;}function
	table_status($F = "", $jc = false) {
		global $e;
		$K = array();foreach (get_rows($jc && $e->server_info >= 5 ? "SELECT TABLE_NAME AS Name, Engine, TABLE_COMMENT AS Comment FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() " . ($F != "" ? "AND TABLE_NAME = " . q($F) : "ORDER BY Name") : "SHOW TABLE STATUS" . ($F != "" ? " LIKE " . q(addcslashes($F, "%_\\")) : "")) as $L) {
			if ($L["Engine"] == "InnoDB") {
	...
	indexes($Q, $f = null) {
		...
		return $K;}function
	foreign_keys($Q) {
		global $e, $Vd;static $ye = '`(?:[^`]|``)+`';
		$K = array();
		$kb = $e->result("SHOW CREATE TABLE " . table($Q), 1);if ($kb) {
			preg_match_all("~CONSTRAINT ($ye) FOREIGN KEY ?\\(((?:$ye,? ?)+)\\) REFERENCES ($ye)(?:\\.($ye))? \\(((?:$ye,? ?)+)\\)(?: ON DELETE ($Vd))?(?: ON UPDATE ($Vd))?~", $kb, $rd, PREG_SET_ORDER);foreach ($rd as $C) {
				preg_match_all("~$ye~", $C[2], $_f);
				preg_match_all("~$ye~", $C[5], $Vf);
				$K[idf_unescape($C[1])] = array("db" => idf_unescape($C[4] != "" ? $C[3] : $C[4]), "table" => idf_unescape($C[4] != "" ? $C[4] : $C[3]), "source" => array_map('idf_unescape', $_f[0]), "target" => array_map('idf_unescape', $Vf[0]), "on_delete" => ($C[6] ? $C[6] : "RESTRICT"), "on_update" => ($C[7] ? $C[7] : "RESTRICT"));}}
		return $K;}function
	view($F) {
		global $e;return
	...
	alter_table($Q, $F, $m, $rc, $bb, $Tb, $Wa, $ya, $te) {
		...
		$Df = ($bb !== null ? " COMMENT=" . q($bb) : "") . ($Tb ? " ENGINE=" . q($Tb) : "") . ($Wa ? " COLLATE " . q($Wa) : "") . ($ya != "" ? " AUTO_INCREMENT=$ya" : "");if ($Q == "") {
			return
	...
	move_tables($S, $Lg, $Vf) {
		$bf = array();foreach (array_merge($S, $Lg) as $Q) {
	...
	copy_tables($S, $Lg, $Vf) {
		queries("SET sql_mode = 'NO_AUTO_VALUE_ON_ZERO'");foreach ($S as $Q) {
			$F = ($Vf == DB ? table("copy_$Q") : idf_escape($Vf) . "." . table($Q));if (!queries("\nDROP TABLE IF EXISTS $F") || !queries("CREATE TABLE $F LIKE " . table($Q)) || !queries("INSERT INTO $F SELECT * FROM " . table($Q))) {
				return
					false;
			}
		}
		foreach ($Lg as $Q) {
			$F = ($Vf == DB ? table("copy_$Q") : idf_escape($Vf) . "." . table($Q));
			$Kg = view($Q);if (!queries("DROP VIEW IF EXISTS $F") || !queries("CREATE VIEW $F AS $Kg[select]")) {
				return
					false;
			}
		}
	...
	routine($F, $U) {
		...
		$sg = "((" . implode("|", array_merge(array_keys($tg), $qa)) . ")\\b(?:\\s*\\(((?:[^'\")]|$Vb)++)\\))?\\s*(zerofill\\s*)?(unsigned(?:\\s+zerofill)?)?)(?:\\s*(?:CHARSET|CHARACTER\\s+SET)\\s*['\"]?([^'\"\\s,]+)['\"]?)?";
		$ye = "\\s*(" . ($U == "FUNCTION" ? "" : $Oc) . ")?\\s*(?:`((?:[^`]|``)*)`\\s*|\\b(\\S+)\\s+)$sg";
		$g = $e->result("SHOW CREATE $U " . idf_escape($F), 2);
		preg_match("~\\(((?:$ye\\s*,?)*)\\)\\s*" . ($U == "FUNCTION" ? "RETURNS\\s+$sg\\s+" : "") . "(.*)~is", $g, $C);
		$m = array();
		preg_match_all("~$ye\\s*,?~is", $C[1], $rd, PREG_SET_ORDER);foreach ($rd as $oe) {
			$F = str_replace("``", "`", $oe[2]) . $oe[3];
			$m[] = array("field" => $F, "type" => strtolower($oe[5]), "length" => preg_replace_callback("~$Vb~s", 'normalize_enum', $oe[6]), "unsigned" => strtolower(preg_replace('~\\s+~', ' ', trim("$oe[8] $oe[7]"))), "null" => 1, "full_type" => $oe[4], "inout" => strtoupper($oe[1]), "collation" => strtolower($oe[9]));}if ($U != "FUNCTION") {
			return
	...
	create_sql($Q, $ya) {
		global $e;
		$K = $e->result("SHOW CREATE TABLE " . table($Q), 1);if (!$ya) {
	...
	unconvert_field($l, $K) {
		...
		return $K;}function
	support($kc) {global $e;return !preg_match("~scheme|sequence|type|view_trigger" . ($e->server_info < 5.1 ? "|event|partitioning" . ($e->server_info < 5 ? "|routine|trigger|view" : "") : "") . "~", $kc);}$y = "sql";
	$tg = array();
	$Hf = array();foreach (array('Numbers' => array("tinyint" => 3, "smallint" => 5, "mediumint" => 8, "int" => 10, "bigint" => 20, "decimal" => 66, "float" => 12, "double" => 21), 'Date and time' => array("date" => 10, "datetime" => 19, "timestamp" => 19, "time" => 10, "year" => 4), 'Strings' => array("char" => 255, "varchar" => 65535, "tinytext" => 255, "text" => 65535, "mediumtext" => 16777215, "longtext" => 4294967295), 'Lists' => array("enum" => 65535, "set" => 64), 'Binary' => array("bit" => 20, "binary" => 255, "varbinary" => 65535, "tinyblob" => 255, "blob" => 65535, "mediumblob" => 16777215, "longblob" => 4294967295), 'Geometry' => array("geometry" => 0, "point" => 0, "linestring" => 0, "polygon" => 0, "multipoint" => 0, "multilinestring" => 0, "multipolygon" => 0, "geometrycollection" => 0)) as $z => $X) {
		$tg += $X;
		$Hf[$z] = array_keys($X);}
	$_g = array("unsigned", "zerofill", "unsigned zerofill");
...
<?php
	...
	selectLinks($Of, $P = "") {
		echo '<p class="links">';
		$od = array("select" => 'Select data');if (support("table") || support("indexes")) {
	...
	selectVal($X, $A, $l, $ke) {
		$K = ($X === null ? "<i>NULL</i>" : (preg_match("~char|binary~", $l["type"]) && !preg_match("~var~", $l["type"]) ? "<code>$X</code>" : $X));if (preg_match('~blob|bytea|raw|file~', $l["type"]) && !is_utf8($X)) {
	...
	selectColumnsPrint($N, $d) {
		...
		$N[""] = array();foreach ($N as $z => $X) {
			...
			echo "<div>" . ($xc || $_c ? "<select name='columns[$t][fun]' onchange='helpClose();" . ($z !== "" ? "" : " this.nextSibling.nextSibling.onchange();") . "'" . on_help("getTarget(event).value && getTarget(event).value.replace(/ |\$/, '(') + ')'", 1) . ">" . optionlist(array(-1 => "") + array_filter(array('Functions' => $xc, 'Aggregation' => $_c)), $X["fun"]) . "</select>" . "($c)" : $c) . "</div>\n";
			$t++;}echo "</div></fieldset>\n";}function
	selectSearchPrint($Z, $d, $w) {
		print_fieldset("search", 'Search', $Z);foreach ($w as $t => $v) {if ($v["type"] == "FULLTEXT") {echo "(<i>" . implode("</i>, <i>", array_map('h', $v["columns"])) . "</i>) AGAINST", " <input type='search' name='fulltext[$t]' value='" . h($_GET["fulltext"][$t]) . "' onchange='selectFieldChange(this.form);'>", checkbox("boolean[$t]", 1, isset($_GET["boolean"][$t]), "BOOL"), "<br>\n";}}$_GET["where"] = (array) $_GET["where"];
		reset($_GET["where"]);
		$Ka = "this.nextSibling.onchange();";for ($t = 0; $t <= count($_GET["where"]); $t++) {list(, $X) = each($_GET["where"]);if (!$X || ("$X[col]$X[val]" != "" && in_array($X["op"], $this->operators))) {echo "<div>" . select_input(" name='where[$t][col]' onchange='$Ka'", $d, $X["col"], "(" . 'anywhere' . ")"), html_select("where[$t][op]", $this->operators, $X["op"], $Ka), "<input type='search' name='where[$t][val]' value='" . h($X["val"]) . "' onchange='" . ($X ? "selectFieldChange(this.form)" : "selectAddRow(this)") . ";' onkeydown='selectSearchKeydown(this, event);' onsearch='selectSearchSearch(this);'></div>\n";}}echo "</div></fieldset>\n";}function
	...
	selectActionPrint($w) {
		...
		$d = array();foreach ($w as $v) {
			if ($v["type"] != "FULLTEXT") {
				$d[reset($v["columns"])] = 1;
			}
		}$d[""] = 1;foreach ($d as $z => $X) {
			json_row($z);
		}
	...
	selectSearchProcess($m, $w) {
		...
		$K = array();foreach ($w as $t => $v) {
			...
			if ("$X[col]$X[val]" != "" && in_array($X["op"], $this->operators)) {
				$db = " $X[op]";if (preg_match('~IN$~', $X["op"])) {
					$Lc = process_length($X["val"]);
	...
	messageQuery($I, $bg) {
		...
		$u = "sql-" . count($Ec[$_GET["db"]]);if (strlen($I) > 1e6) {
	...
	processInput($l, $Y, $q = "") {
		...
		$F = $l["field"];
		$K = q($Y);if (preg_match('~^(now|getdate|uuid)$~', $q)) {
			$K = "$q()";
	...
	dumpOutput() {
		$K = array('text' => 'open', 'file' => 'save');if (function_exists('gzencode')) {
			$K['gz'] = 'gzip';
		}
		return $K;}function
	dumpFormat() {
	...
	dumpTable($Q, $If, $Yc = 0) {
		if ($_POST["format"] != "sql") {
			...
			set_utf8mb4($g);if ($If && $g) {
				if ($If == "DROP+CREATE" || $Yc == 1) {
			...
			if ($_POST["format"] == "sql") {
				...
				$lc = ($Q != '' ? 'fetch_assoc' : 'fetch_row');while ($L = $J->$lc()) {
					if (!$ad) {
						...
						foreach ($L as $z => $X) {
							$l = $m[$z];
							$L[$z] = ($X !== null ? unconvert_field($l, preg_match('~(^|[^o])int|float|double|decimal~', $l["type"]) && $X != '' ? $X : q($X)) : "NULL");}$lf = ($td ? "\n" : " ") . "(" . implode(",\t", $L) . ")";if (!$Ia) {
							$Ia = $Qc . $lf;
	...
	dumpHeaders($Ic, $Ed = false) {
		$me = $_POST["output"];
		$gc = (preg_match('~sql~', $_POST["format"]) ? "sql" : ($Ed ? "tar" : "csv"));
		header("Content-Type: " . ($me == "gz" ? "application/x-gzip" : ($gc == "tar" ? "application/x-tar" : ($gc == "sql" || $me != "file" ? "text/plain" : "text/csv") . "; charset=utf-8")));if ($me == "gz") {
...
';}$this->databasesPrint($Dd);if (DB == "" || !$Dd) {
...
';
		...
		echo "<span title='" . 'database' . "'>DB</span>: " . ($h ? "<select name='db'$rb>" . optionlist(array("" => "") + $h, DB) . "</select>" : '<input name="db" value="' . h(DB) . '" autocapitalize="off">'), "<input type='submit' value='" . 'Use' . "'" . ($h ? " class='hidden'" : "") . ">\n";if ($Dd != "db" && DB != "" && $e->select_db(DB)) {}echo (isset($_GET["sql"]) ? '<input type="hidden" name="sql" value="">' : (isset($_GET["schema"]) ? '<input type="hidden" name="schema" value="">' : (isset($_GET["dump"]) ? '<input type="hidden" name="dump" value="">' : (isset($_GET["privileges"]) ? '<input type="hidden" name="privileges" value="">' : "")))), "</p></form>\n";}function
	tablesPrint($S) {
		echo "<p id='tables' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";foreach ($S as $Q => $Df) {
			echo '<a href="' . h(ME) . 'select=' . urlencode($Q) . '"' . bold($_GET["select"] == $Q || $_GET["edit"] == $Q, "select") . ">" . 'select' . "</a> ";
			$F = $this->tableName($Df);
			echo (support("table") || support("indexes") ? '<a href="' . h(ME) . 'table=' . urlencode($Q) . '"' . bold(in_array($Q, array($_GET["table"], $_GET["create"], $_GET["indexes"], $_GET["foreign"], $_GET["trigger"])), (is_view($Df) ? "view" : ""), "structure") . " title='" . 'Show structure' . "'>$F</a>" : "<span>$F</span>") . "<br>\n";}}}$b = (function_exists('adminer_object') ? adminer_object() : new
	Adminer);if ($b->operators === null) {
	$b->operators = $Zd;
}
function
page_header($eg, $k = "", $Ha = array(), $fg = "") {
	global $ca, $ga, $b, $Eb, $y;
	page_headers();if (is_ajax() && $k) {page_messages($k);exit;}
	$gg = $eg . ($fg != "" ? ": $fg" : "");
	$hg = strip_tags($gg . (SERVER != "" && SERVER != "localhost" ? h(" - " . SERVER) : "") . " - " . $b->name());
	echo '<!DOCTYPE html>
...
<title>', $hg, '</title>
<link rel="stylesheet" type="text/css" href="', h(preg_replace("~\\?.*~", "", ME)) . "?file=default.css&amp;version=4.2.4&amp;driver=mysql", '">
<script type="text/javascript" src="', h(preg_replace("~\\?.*~", "", ME)) . "?file=functions.js&amp;version=4.2.4&amp;driver=mysql", '"></script>
';if ($b->head()) {echo '<link rel="shortcut icon" type="image/x-icon" href="', h(preg_replace("~\\?.*~", "", ME)) . "?file=favicon.ico&amp;version=4.2.4&amp;driver=mysql", '">
<link rel="apple-touch-icon" href="', h(preg_replace("~\\?.*~", "", ME)) . "?file=favicon.ico&amp;version=4.2.4&amp;driver=mysql", '">
...
';if ($Ha !== null) {
		...
		$O = (SERVER != "" ? h(SERVER) : 'Server');if ($Ha === false) {
			echo "$O\n";
		} else {
	...
	restart_session();
	page_messages($k);
	$h = &get_session("dbs");if (DB != "" && $h && !in_array(DB, $h, true)) {
		$h = null;
	}
...
page_headers() {
	...
	header("Cache-Control: no-cache");if ($b->headers()) {
		header("X-Frame-Options: deny");
		header("X-XSS-Protection: 0");}}function
page_messages($k) {
	$Bg = preg_replace('~^[^?]*~', '', $_SERVER["REQUEST_URI"]);
	$Bd = $_SESSION["messages"][$Bg];if ($Bd) {echo "<div class='message'>" . implode("</div>\n<div class='message'>", $Bd) . "</div>\n";unset($_SESSION["messages"][$Bg]);}if ($k) {
...
long2str($W, $Ng) {
	...
	return $lf;}function
str2long($lf, $Ng) {
	$W = array_values(unpack('V*', str_pad($lf, 4 * ceil(strlen($lf) / 4), "\0")));if ($Ng) {
		$W[] = strlen($lf);
	}
	return $W;}function
xxtea_mx($Sg, $Rg, $Lf, $Zc) {
	return
...
encrypt_string($Ff, $z) {
	...
	$H = floor(6 + 52 / ($E + 1));
	$Lf = 0;while ($H-- > 0) {
		$Lf = int32($Lf + 0x9E3779B9);
		$Kb = $Lf >> 2 & 3;for ($ne = 0; $ne < $E; $ne++) {
			$Rg = $W[$ne + 1];
...
add_invalid_login() {
	...
	rewind($p);
	fwrite($p, $tf);
	ftruncate($p, strlen($tf));
	flock($p, LOCK_UN);
	fclose($p);}$xa = $_POST["auth"];if ($xa) {
	$Tc = unserialize(@file_get_contents(get_temp_dir() . "/adminer.invalid"));
	$Sc = $Tc[$b->bruteForceKey()];
	$Kd = ($Sc[1] > 30 ? $Sc[0] - time() : 0);if ($Kd > 0) {
	...
	$O = $xa["server"];
	$V = $xa["username"];
	$we = (string) $xa["password"];
	$i = $xa["db"];
	set_password($Jg, $O, $V, $we);
	$_SESSION["db"][$Jg][$O][$V][$i] = true;if ($xa["permanent"]) {
		$z = base64_encode($Jg) . "-" . base64_encode($O) . "-" . base64_encode($V) . "-" . base64_encode($i);
		$Je = $b->permanentLogin(true);
		$ze[$z] = "$z:" . base64_encode($Je ? encrypt_string($we, $Je) : "");
		cookie("adminer_permanent", implode(" ", $ze));}if (count($_POST) == 1 || DRIVER != $Jg || SERVER != $O || $_GET["username"] !== $V || DB != $i) {
	...
	if ($Dc && !verify_token()) {
		...
		unset_permanent();
		redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~', '', ME), 0, -1), 'Logout successful.');}} elseif ($ze && !$_SESSION["pwds"]) {
	session_regenerate_id();
	$Je = $b->permanentLogin();foreach ($ze as $z => $X) {
		list(, $Qa) = explode(":", $X);list($Jg, $O, $V, $i) = array_map('base64_decode', explode("-", $z));
		set_password($Jg, $O, $V, decrypt_string(base64_decode($Qa), $Je));
		$_SESSION["db"][$Jg][$O][$V][$i] = true;}}
function
unset_permanent() {
	global $ze;foreach ($ze as $z => $X) {
		list($Jg, $O, $V, $i) = array_map('base64_decode', explode("-", $z));if ($Jg == DRIVER && $O == SERVER && $V == $_GET["username"] && $i == DB) {
			unset($ze[$z]);
		}
	}
...
auth_error($k) {
	global $b, $Dc;
	$k = h($k);
	$wf = session_name();if (isset($_GET["username"])) {
		header("HTTP/1.1 403 Forbidden");if (($_COOKIE[$wf] || $_GET[$wf]) && !$Dc) {
			$k = 'Session expired, please login again.';
		} else {
			add_invalid_login();
			$we = get_password();if ($we !== null) {
				if ($we === false) {
			...
			unset_permanent();}}if (!$_COOKIE[$wf] && $_GET[$wf] && ini_bool("session.use_only_cookies")) {
	...
	$e = connect();}
$j = new
Min_Driver($e);if (!is_object($e) || !$b->login($_GET["username"], get_password())) {
	auth_error((is_string($e) ? $e : 'Invalid credentials.'));
}
if ($xa && $_POST["token"]) {
	$_POST["token"] = $T;
}
		...
		if (!$t) {
			echo "<table cellspacing='0' class='nowrap'>\n", "<thead><tr>";for ($x = 0; $x < count($L); $x++) {
				...
				$K[$l->table] = $fe;if ($ge && $y == "sql") {
					...
					if (!isset($w[$fe])) {
						$w[$fe] = array();foreach (indexes($fe, $f) as $v) {
							if ($v["type"] == "PRIMARY") {
...
referencable_primary($rf) {
	$K = array();foreach (table_status('', true) as $Pf => $Q) {
		if ($Pf != $rf && fk_support($Q)) {
			foreach (fields($Pf) as $l) {
				if ($l["primary"]) {
					if ($K[$Pf]) {unset($K[$Pf]);
						break;}
					$K[$Pf] = $l;}}}}
...
edit_type($z, $l, $Xa, $o = array()) {
	global $Hf, $tg, $_g, $Vd;
	$U = $l["type"];
	echo '<td><select name="', h($z), '[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);"', on_help("getTarget(event).value", 1), '>';if ($U && !isset($tg[$U]) && !isset($o[$U])) {
		array_unshift($Hf, $U);
	}
	if ($o) {
		$Hf['Foreign keys'] = $o;
...
<td><input name="', h($z), '[length]" value="', h($l["length"]), '" size="3" onfocus="editingLengthFocus(this);"', (!$l["length"] && preg_match('~var(char|binary)$~', $U) ? " class='required'" : ""), ' onchange="editingLengthChange(this);" onkeyup="this.onchange();"><td class="options">';
...
';if ($U == "PROCEDURE") {echo '<td>&nbsp;';}
	echo '<th>', ($U == "TABLE" ? 'Column name' : 'Parameter name'), '<td>Type<textarea id="enum-edit" rows="4" cols="12" wrap="off" style="display: none;" onblur="editingLengthBlur(this);"></textarea>
<td>Length
<td>Options
...
', (support("comment") ? "<td" . ($cb ? "" : " class='hidden'") . ">" . 'Comment' : "");}
	echo '<td>', "<input type='image' class='icon' name='add[" . (support("move_col") ? 0 : count($m)) . "]' src='" . h(preg_replace("~\\?.*~", "", ME)) . "?file=plus.gif&amp;version=4.2.4&amp;driver=mysql' alt='+' title='" . 'Add next' . "'>", '<script type="text/javascript">row_count = ', count($m), ';</script>
</thead>
<tbody onkeydown="return editingKeydown(event);">
';foreach ($m as $t => $l) {
		$t++;
		$he = $l[($_POST ? "orig" : "field")];
		$Ab = (isset($_POST["add"][$t - 1]) || (isset($l["field"]) && !$_POST["drop_col"][$t])) && (support("drop_col") || $he == "");
		echo '<tr', ($Ab ? "" : " style='display: none;'"), '>
', ($U == "PROCEDURE" ? "<td>" . html_select("fields[$t][inout]", explode("|", $Oc), $l["inout"]) : ""), '<th>';if ($Ab) {echo '<input name="fields[', $t, '][field]" value="', h($l["field"]), '" onchange="editingNameChange(this);', ($l["field"] != "" || count($m) > 1 ? '' : ' editingAddRow(this);" onkeyup="if (this.value) editingAddRow(this);'), '" maxlength="64" autocapitalize="off">';}
		echo '<input type="hidden" name="fields[', $t, '][orig]" value="', h($he), '">
';
		edit_type("fields[$t]", $l, $Xa, $o);if ($U == "TABLE") {
			echo '<td>', checkbox("fields[$t][null]", 1, $l["null"], "", "", "block"), '<td><label class="block"><input type="radio" name="auto_increment_col" value="', $t, '"';if ($l["auto_increment"]) {echo ' checked';} ?> onclick="var field = this.form['fields[' + this.value + '][field]']; if (!field.value) { field.value = 'id'; field.onchange(); }"></label><td><?php
echo
...
', (support("comment") ? "<td" . ($cb ? "" : " class='hidden'") . "><input name='fields[$t][comment]' value='" . h($l["comment"]) . "' maxlength='" . ($e->server_info >= 5.5 ? 1024 : 255) . "'>" : "");}
		echo "<td>", (support("move_col") ? "<input type='image' class='icon' name='add[$t]' src='" . h(preg_replace("~\\?.*~", "", ME)) . "?file=plus.gif&amp;version=4.2.4&amp;driver=mysql' alt='+' title='" . 'Add next' . "' onclick='return !editingAddRow(this, 1);'>&nbsp;" . "<input type='image' class='icon' name='up[$t]' src='" . h(preg_replace("~\\?.*~", "", ME)) . "?file=up.gif&amp;version=4.2.4&amp;driver=mysql' alt='^' title='" . 'Move up' . "'>&nbsp;" . "<input type='image' class='icon' name='down[$t]' src='" . h(preg_replace("~\\?.*~", "", ME)) . "?file=down.gif&amp;version=4.2.4&amp;driver=mysql' alt='v' title='" . 'Move down' . "'>&nbsp;" : ""), ($he == "" || support("drop_col") ? "<input type='image' class='icon' name='drop_col[$t]' src='" . h(preg_replace("~\\?.*~", "", ME)) . "?file=cross.gif&amp;version=4.2.4&amp;driver=mysql' alt='x' title='" . 'Remove' . "' onclick=\"return !editingRemoveRow(this, 'fields\$1[field]');\">" : ""), "\n";}}function
process_fields(&$m) {
	ksort($m);
	$Od = 0;if ($_POST["up"]) {
		...
		$uc = false;foreach ($m as $z => $l) {
			...
			$Od++;}} elseif ($_POST["add"]) {
		$m = array_values($m);
		array_splice($m, key($_POST["add"]), 0, array(array()));} elseif (!$_POST["drop_col"]) {
		return
			false;
	}
	return
...
drop_create($Fb, $g, $Gb, $Yf, $Hb, $B, $Ad, $zd, $_d, $Rd, $Id) {
	if ($_POST["drop"]) {
		...
		$lb = queries($g);
		queries_redirect($B, $zd, $lb && queries($Fb));if ($lb) {
			queries($Gb);
		}
	} else {
...
create_trigger($Ud, $L) {
	global $y;
	$dg = " $L[Timing] $L[Event]" . ($L["Event"] == "UPDATE OF" ? " " . idf_escape($L["Of"]) : "");return "CREATE TRIGGER " . idf_escape($L["Trigger"]) . ($y == "mssql" ? $Ud . $dg : $dg . $Ud) . rtrim(" $L[Type]\n$L[Statement]", ";") . ";";}function
create_routine($if, $L) {
	global $Oc;
	$P = array();
	$m = (array) $L["fields"];
	ksort($m);foreach ($m as $l) {
		if ($l["field"] != "") {
...
tar_file($nc, $ig) {
	$K = pack("a100a8a8a8a12a12", $nc, 644, 0, 0, decoct($ig->size), decoct(time()));
...
connect_error() {
	global $b, $e, $T, $k, $Eb;if (DB != "") {
		...
		echo "<p class='links'>\n";foreach (array('database' => 'Create new database', 'privileges' => 'Privileges', 'processlist' => 'Process list', 'variables' => 'Variables', 'status' => 'Status') as $z => $X) {
			if (support($z)) {
		...
		$h = $b->databases();if ($h) {
			...
			$h = ($_GET["dbsize"] ? count_tables($h) : array_flip($h));foreach ($h as $i => $S) {
				$hf = h(ME) . "db=" . urlencode($i);
				echo "<tr" . odd() . ">" . (support("database") ? "<td>" . checkbox("db[]", $i, in_array($i, (array) $_POST["db"])) : ""), "<th><a href='$hf'>" . h($i) . "</a>";
				$Wa = nbsp(db_collation($i, $Xa));
				echo "<td>" . (support("database") ? "<a href='$hf" . ($of ? "&amp;ns=" : "") . "&amp;database=' title='" . 'Alter database' . "'>$Wa</a>" : $Wa), "<td align='right'><a href='$hf&amp;schema=' id='tables-" . h($i) . "' title='" . 'Database schema' . "'>" . ($_GET["dbsize"] ? $S : "?") . "</a>", "<td align='right' id='size-" . h($i) . "'>" . ($_GET["dbsize"] ? db_size($i) : "?"), "\n";}
...
$Oc = "IN|OUT|INOUT";if (isset($_GET["select"]) && ($_POST["edit"] || $_POST["clone"]) && !$_POST["save"]) {
...
if (isset($_GET["download"])) {
	...
	page_header(($m && is_view($R) ? 'View' : 'Table') . ": " . h($a), $k);
	$b->selectLinks($R);
	$bb = $R["Comment"];if ($bb != "") {
		echo "<p>" . 'Comment' . ": " . h($bb) . "\n";
	}
	if ($m) {
		...
		if (support("indexes")) {
			...
			$w = indexes($a);if ($w) {
				echo "<table cellspacing='0'>\n";foreach ($w as $F => $v) {
					ksort($v["columns"]);
					$Ie = array();foreach ($v["columns"] as $z => $X) {
			...
			$o = foreign_keys($a);if ($o) {
				echo "<table cellspacing='0'>\n", "<thead><tr><th>" . 'Source' . "<td>" . 'Target' . "<td>" . 'ON DELETE' . "<td>" . 'ON UPDATE' . "<td>&nbsp;</thead>\n";foreach ($o as $F => $n) {echo "<tr title='" . h($F) . "'>", "<th><i>" . implode("</i>, <i>", array_map('h', $n["source"])) . "</i>", "<td><a href='" . h($n["db"] != "" ? preg_replace('~db=[^&]*~', "db=" . urlencode($n["db"]), ME) : ($n["ns"] != "" ? preg_replace('~ns=[^&]*~', "ns=" . urlencode($n["ns"]), ME) : ME)) . "table=" . urlencode($n["table"]) . "'>" . ($n["db"] != "" ? "<b>" . h($n["db"]) . "</b>." : "") . ($n["ns"] != "" ? "<b>" . h($n["ns"]) . "</b>." : "") . h($n["table"]) . "</a>", "(<i>" . implode("</i>, <i>", array_map('h', $n["target"])) . "</i>)", "<td>" . nbsp($n["on_delete"]) . "\n", "<td>" . nbsp($n["on_update"]) . "\n", '<td><a href="' . h(ME . 'foreign=' . urlencode($a) . '&name=' . urlencode($F)) . '">' . 'Alter' . '</a>';}
				echo "</table>\n";}
	...
	preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~', $ea, $rd, PREG_SET_ORDER);foreach ($rd as $t => $C) {
		$Qf[$C[1]] = array($C[2], $C[3]);
		$Rf[] = "\n\t'" . js_escape($C[1]) . "': [ $C[2], $C[3] ]";}
	$kg = 0;
	$Ca = -1;
	$nf = array();
	$Ye = array();
	$jd = array();foreach (table_status('', true) as $Q => $R) {
		if (is_view($R)) {
			continue;
		}

		$Be = 0;
		$nf[$Q]["fields"] = array();foreach (fields($Q) as $F => $l) {
			$Be += 1.25;
			$l["pos"] = $Be;
			$nf[$Q]["fields"][$F] = $l;}
		$nf[$Q]["pos"] = ($Qf[$Q] ? $Qf[$Q] : array($kg, 0));foreach ($b->foreignKeys($Q) as $X) {
			if (!$X["db"]) {
				...
				$jd[(string) $hd] = true;}}
		$kg = max($kg, $nf[$Q]["pos"][0] + 2.5 + $Be);}
	echo '<div id="schema" style="height: ', $kg, 'em;" onselectstart="return false;">
<script type="text/javascript">
...
';foreach ($nf as $F => $Q) {
		...
		foreach ((array) $Ye[$F] as $Wf => $Ze) {
			foreach ($Ze as $hd => $d) {
				$id = $hd - $Qf[$F][1];
				$t = 0;foreach ($d as $Vf) {
					echo "\n<div class='references' title='" . h($Wf) . "' id='refd$hd-" . ($t++) . "' style='left: $id" . "em; top: " . $Q["fields"][$Vf]["pos"] . "em; height: 1.25em; background: url(" . h(preg_replace("~\\?.*~", "", ME)) . "?file=arrow.gif) no-repeat right center;&amp;version=4.2.4&amp;driver=mysql'><div style='height: .5em; border-bottom: 1px solid Gray; width: " . (-$id) . "em;'></div></div>";
				}
			}}
		echo "\n</div>\n";}
	foreach ($nf as $F => $Q) {
...
";
		...
		$If = $_POST["db_style"];
		$h = array(DB);if (DB == "") {
			$h = $_POST["databases"];if (is_string($h)) {
				$h = explode("\n", rtrim(str_replace("\r", "", $h), "\n"));
			}
		}
		foreach ((array) $h as $i) {
			$b->dumpDatabase($i);if ($e->select_db($i)) {
				if ($Wc && preg_match('~CREATE~', $If) && ($g = $e->result("SHOW CREATE DATABASE " . idf_escape($i), 1))) {
					set_utf8mb4($g);if ($If == "DROP+CREATE") {
					...
					$le = "";if ($_POST["routines"]) {
						foreach (array("FUNCTION", "PROCEDURE") as $if) {
							foreach (get_rows("SHOW $if STATUS WHERE Db = " . q($i), null, "-- ") as $L) {
								$g = remove_definer($e->result("SHOW CREATE $if " . idf_escape($L["Name"]), 2));
								set_utf8mb4($g);
...
';
	$Ge = array();if (DB != "") {
		...
		$Lg = "";
		$Tf = tables_list();foreach ($Tf as $F => $U) {
			$Fe = preg_replace('~_.*~', '', $F);
			$Na = ($a == "" || $a == (substr($a, -1) == "%" ? "$Fe%" : $F));
			$Ie = "<tr><td>" . checkbox("tables[]", $F, $Na, $F, "checkboxClick(event, this); formUncheck('check-tables');", "block");if ($U !== null && !preg_match('~table~i', $U)) {
				$Lg .= "$Ie\n";
			} else {
...
';
	...
	$Ec = &$Fc[DB];if (!$k && $_POST["clear"]) {
		$Ec = array();
		redirect(remove_from_uri("history"));}
	page_header((isset($_GET["import"]) ? 'Import' : 'SQL command'), $k);if (!$k && $_POST) {
		$p = false;if (!isset($_GET["import"])) {
			$I = $_POST["query"];
		} elseif ($_POST["webfile"]) {
			$p = @fopen((file_exists("adminer.sql") ? "adminer.sql" : "compress.zlib://adminer.sql.gz"), "rb");
			$I = ($p ? fread($p, 1e6) : false);} else {
				...
				if (!$Od && preg_match("~^$Af*DELIMITER\\s+(\\S+)~i", $I, $C)) {
					...
					preg_match('(' . preg_quote($wb) . "\\s*|$qe)", $I, $C, PREG_OFFSET_CAPTURE, $Od);list($uc, $Be) = $C[0];if (!$uc && $p && !feof($p)) {
						...
						$Od = $Be + strlen($uc);if ($uc && rtrim($uc) != $wb) {
							...
							$H = substr($I, 0, $Be);
							$ab++;
							$Ie = "<pre id='sql-$ab'><code class='jush-$y'>" . shorten_utf8(trim($H), 1000) . "</code></pre>\n";if ($y == "sqlite" && preg_match("~^$Af*ATTACH\b~i", $H, $C)) {
								echo $Ie, "<p class='error'>" . 'ATTACH queries are not supported.' . "\n";
								...
								do {
									$J = $e->store_result();
									$bg = " <span class='time'>(" . format_time($Cf) . ")</span>" . (strlen($H) < 1000 ? " <a href='" . h(ME) . "sql=" . urlencode(trim($H)) . "'>" . 'Edit' . "</a>" : "");if ($e->error) {
										echo ($_POST["only_errors"] ? $Ie : ""), "<p class='error'>" . 'Error in query' . ($e->errno ? " ($e->errno)" : "") . ": " . error() . "\n";
										$Xb[] = " <a href='#sql-$ab'>$ab</a>";if ($_POST["error_stops"]) {
											break
												2;
										}
										...
										$_ = $_POST["limit"];
										$ge = select($J, $f, array(), $_);if (!$_POST["only_errors"]) {
											echo "<form action='' method='post'>\n";
											$Ld = $J->num_rows;
											echo "<p>" . ($Ld ? ($_ && $Ld > $_ ? sprintf('%d / ', $_) : "") . lang(array('%d row', '%d rows'), $Ld) : ""), $bg;
											$u = "export-$ab";
											$fc = ", <a href='#$u' onclick=\"return !toggle('$u');\">" . 'Export' . "</a><span id='$u' class='hidden'>: " . html_select("output", $b->dumpOutput(), $la["output"]) . " " . html_select("format", $Jb, $la["format"]) . "<input type='hidden' name='query' value='" . h($H) . "'>" . " <input type='submit' name='export' value='" . 'Export' . "'><input type='hidden' name='token' value='$T'></span>\n";if ($f && preg_match("~^($Af|\\()*SELECT\\b~isU", $H) && ($ec = explain($f, $H))) {
												$u = "explain-$ab";
									...
									$Cf = microtime(true);} while ($e->next_result());}
							$I = substr($I, $Od);
							$Od = 0;}}}}if ($Sb) {
...
';
	$cc = "<input type='submit' value='" . 'Execute' . "' title='Ctrl+Enter'>";if (!isset($_GET["import"])) {
		$H = $_GET["sql"];if ($_POST) {
			$H = $_POST["query"];
...
';} elseif (isset($_GET["edit"])) {
	...
	$Ag = (isset($_GET["select"]) ? $_POST["edit"] : $Z);foreach ($m as $F => $l) {
		if (!isset($l["privileges"][$Ag ? "update" : "insert"]) || $b->fieldName($l) == "") {
			unset($m[$F]);
		}
		...
		$Re = "\nWHERE $Z";if (isset($_POST["delete"])) {
			...
			$P = array();foreach ($m as $F => $l) {
				$X = process_input($l);if ($X !== false && $X !== null) {
					$P[idf_escape($F)] = $X;
				}
			}if ($Ag) {
				if (!$P) {
				...
				queries_redirect($B, 'Item has been updated.', $j->update($a, $P, $Re, !$wg));if (is_ajax()) {
					page_headers();
					page_messages($k);exit;}} else {
				$J = $j->insert($a, $P);
				$gd = ($J ? last_id() : 0);
				queries_redirect($B, sprintf('Item%s has been inserted.', ($gd ? " $gd" : "")), $J);}}}
	...
	edit_form($a, $m, $L, $Ag);} elseif (isset($_GET["create"])) {
	$a = $_GET["create"];
	$re = array();foreach (array('HASH', 'LINEAR HASH', 'KEY', 'LINEAR KEY', 'RANGE', 'LIST') as $z) {
		$re[$z] = $z;
	}

	$Xe = referencable_primary($a);
	$o = array();foreach ($Xe as $Pf => $l) {
	...
	if ($_POST && !process_fields($L["fields"]) && !$k) {
		if ($_POST["drop"]) {
			...
			ksort($L["fields"]);
			$ie = reset($je);
			$pa = " FIRST";foreach ($L["fields"] as $z => $l) {
				$n = $o[$l["type"]];
				$rg = ($n !== null ? $Xe[$n] : $l);if ($l["field"] != "") {
					if (!$l["has_default"]) {
						$l["default"] = null;
					}
					...
					$Ne = process_field($l, $rg);
					$ra[] = array($l["orig"], $Ne, $pa);if ($Ne != process_field($ie, $ie)) {
						$m[] = array($l["orig"], $Ne, $pa);if ($l["orig"] != "" || $pa) {
							$Dg = true;
						}
					}if ($n !== null) {
					...
					$pa = " AFTER " . idf_escape($l["field"]);} elseif ($l["orig"] != "") {
					$Dg = true;
					$m[] = array($l["orig"]);}if ($l["orig"] != "") {
					$ie = next($je);if (!$ie) {
						$pa = "";
					}
				}}
	...
	page_header(($a != "" ? 'Alter table' : 'Create table'), $k, array("table" => $a), h($a));if (!$_POST) {
		$L = array("Engine" => $_COOKIE["adminer_engine"], "fields" => array(array("field" => "", "type" => (isset($tg["int"]) ? "int" : (isset($tg["integer"]) ? "integer" : "")))), "partition_names" => array(""));if ($a != "") {
			...
			foreach ($je as $l) {
				...
				$L["fields"][] = $l;}if (support("partitioning")) {
				$wc = "FROM information_schema.PARTITIONS WHERE TABLE_SCHEMA = " . q(DB) . " AND TABLE_NAME = " . q($a);
				$J = $e->query("SELECT PARTITION_METHOD, PARTITION_ORDINAL_POSITION, PARTITION_EXPRESSION $wc ORDER BY PARTITION_ORDINAL_POSITION DESC LIMIT 1");list($L["partition_by"], $L["partitions"], $L["partition"]) = $J->fetch_row();
				$ue = get_key_vals("SELECT PARTITION_NAME, PARTITION_DESCRIPTION $wc AND PARTITION_NAME != '' ORDER BY PARTITION_ORDINAL_POSITION");
				$ue[""] = "";
...
', checkbox("defaults", 1, true, 'Default values', "columnShow(this.checked, 5)", "jsonly");if (!$_POST["defaults"]) {echo '<script type="text/javascript">editingHideDefaults()</script>';}
...
';}
	echo '
';if ($a != "") {echo '<input type="submit" name="drop" value="Drop"', confirm(), '>';}if (support("partitioning")) {
...
';} elseif (isset($_GET["indexes"])) {
	$a = $_GET["indexes"];
	$Mc = array("PRIMARY", "UNIQUE", "INDEX");
	$R = table_status($a, true);if (preg_match('~MyISAM|M?aria' . ($e->server_info >= 5.6 ? '|InnoDB' : '') . '~i', $R["Engine"])) {
		$Mc[] = "FULLTEXT";
	}
	...
	$L = $_POST;if ($_POST && !$k && !$_POST["add"] && !$_POST["drop_col"]) {
		$sa = array();foreach ($L["indexes"] as $v) {
			$F = $v["name"];if (in_array($v["type"], $Mc)) {
				$d = array();
				$md = array();
				$yb = array();
				$P = array();
...
<th>Name
<th><noscript><input type='image' class='icon' name='add[0]' src='" . h(preg_replace("~\\?.*~", "", ME)) . "?file=plus.gif&amp;version=4.2.4&amp;driver=mysql' alt='+' title='Add next'></noscript>&nbsp;
</thead>
<?php
if ($He) {
	...
	$x = 1;foreach ($L["indexes"] as $v) {
		if (!$_POST["drop_col"] || $x != key($_POST["drop_col"])) {
			...
			$t = 1;foreach ($v["columns"] as $z => $c) {
				...
				$t++;}
			echo "<td><input name='indexes[$x][name]' value='" . h($v["name"]) . "' autocapitalize='off'>\n", "<td><input type='image' class='icon' name='drop_col[$x]' src='" . h(preg_replace("~\\?.*~", "", ME)) . "?file=cross.gif&amp;version=4.2.4&amp;driver=mysql' alt='x' title='" . 'Remove' . "' onclick=\"return !editingRemoveRow(this, 'indexes\$1[type]');\">\n";}
		$x++;}
	echo '</table>
<p>
<input type="submit" value="Save">
...
if (DB != "") {
		...
		echo "<input type='image' class='icon' name='add' src='" . h(preg_replace("~\\?.*~", "", ME)) . "?file=plus.gif&amp;version=4.2.4&amp;driver=mysql' alt='+' title='" . 'Add next' . "'>\n";
	}
...
';} elseif (isset($_GET["call"])) {
	...
	$if = routine($da, (isset($_GET["callf"]) ? "FUNCTION" : "PROCEDURE"));
	$Lc = array();
	$le = array();foreach ($if["fields"] as $t => $l) {
		...
		echo "<p><code class='jush-$y'>" . h($I) . "</code> <a href='" . h(ME) . "sql=" . urlencode($I) . "'>" . 'Edit' . "</a>\n";if (!$e->multi_query($I)) {
			...
			$f = connect();if (is_object($f)) {
				$f->select_db(DB);
			}
			do {
				$J = $e->store_result();if (is_object($J)) {
					select($J, $f);
				} else {
...
';if ($Lc) {
		echo "<table cellspacing='0'>\n";foreach ($Lc as $z) {
			...
			input($l, $Y, (string) $_POST["function"][$F]);
			echo "\n";}
		echo "</table>\n";}
	echo '<p>
...
';} elseif (isset($_GET["foreign"])) {
	$a = $_GET["foreign"];
	$F = $_GET["name"];
	$L = $_POST;if ($_POST && !$k && !$_POST["add"] && !$_POST["change"] && !$_POST["change-js"]) {
		$D = ($_POST["drop"] ? 'Foreign key has been dropped.' : ($F != "" ? 'Foreign key has been altered.' : 'Foreign key has been created.'));
		$B = ME . "table=" . urlencode($a);
		$L["source"] = array_filter($L["source"], 'strlen');
		ksort($L["source"]);
		$Vf = array();foreach ($L["source"] as $z => $X) {
			$Vf[$z] = $L["target"][$z];
		}
	...
	page_header('Foreign key', $k, array("table" => $a), h($a));if ($_POST) {
		...
		$L["table"] = $a;
		$L["source"] = array("");}
	$_f = array_keys(fields($a));
	$Vf = ($a === $L["table"] ? $_f : array_keys(fields($L["table"])));
...
';
		$x = 0;foreach ($L["source"] as $z => $X) {
			echo "<tr>", "<td>" . html_select("source[" . (+$z) . "]", array(-1 => "") + $_f, $X, ($x == count($L["source"]) - 1 ? "foreignAddRow(this);" : 1)), "<td>" . html_select("target[" . (+$z) . "]", $Vf, $L["target"][$z]);
			$x++;}
		echo '</table>
<p>
...
';}if ($F != "") {echo '<input type="submit" name="drop" value="Drop"', confirm(), '>';}
	echo '<input type="hidden" name="token" value="', $T, '">
</form>
';} elseif (isset($_GET["view"])) {
	...
	$L = $_POST;if ($_POST && !$k) {
		...
		$D = 'View has been altered.';if ($_GET["materialized"]) {
			...
			$Xf = $F . "_adminer_" . uniqid();
			drop_create("DROP $U " . table($a), "CREATE $U " . table($F) . $ua, "DROP $U " . table($F), "CREATE $U " . table($Xf) . $ua, "DROP $U " . table($Xf), ($_POST["drop"] ? substr(ME, 0, -1) : $B), 'View has been dropped.', $D, 'View has been created.', $a, $F);}}if (!$_POST && $a != "") {
		$L = view($a);
		$L["name"] = $a;if (!$k) {
			$k = error();
		}
...
<p>';
	...
	echo '<p>
<input type="submit" value="Save">
';if ($_GET["view"] != "") {echo '<input type="submit" name="drop" value="Drop"', confirm(), '>';}
	echo '<input type="hidden" name="token" value="', $T, '">
</form>
';} elseif (isset($_GET["event"])) {
	$aa = $_GET["event"];
	$Rc = array("YEAR", "QUARTER", "MONTH", "DAY", "HOUR", "MINUTE", "WEEK", "SECOND", "YEAR_MONTH", "DAY_HOUR", "DAY_MINUTE", "DAY_SECOND", "HOUR_MINUTE", "HOUR_SECOND", "MINUTE_SECOND");
	$Ef = array("ENABLED" => "ENABLE", "DISABLED" => "DISABLE", "SLAVESIDE_DISABLED" => "DISABLE ON SLAVE");
	$L = $_POST;if ($_POST && !$k) {
		if ($_POST["drop"]) {
			query_redirect("DROP EVENT " . idf_escape($aa), substr(ME, 0, -1), 'Event has been dropped.');
		} elseif (in_array($L["INTERVAL_FIELD"], $Rc) && isset($Ef[$L["STATUS"]])) {
			...
			queries_redirect(substr(ME, 0, -1), ($aa != "" ? 'Event has been altered.' : 'Event has been created.'), queries(($aa != "" ? "ALTER EVENT " . idf_escape($aa) . $mf . ($aa != $L["EVENT_NAME"] ? "\nRENAME TO " . idf_escape($L["EVENT_NAME"]) : "") : "CREATE EVENT " . idf_escape($L["EVENT_NAME"]) . $mf) . "\n" . $Ef[$L["STATUS"]] . " COMMENT " . q($L["EVENT_COMMENT"]) . rtrim(" DO\n$L[EVENT_DEFINITION]", ";") . ";"));}}
	page_header(($aa != "" ? 'Alter event' . ": " . h($aa) : 'Create event'), $k);if (!$L && $aa != "") {
...
<p>';
	...
	echo '<p>
<input type="submit" value="Save">
';if ($aa != "") {echo '<input type="submit" name="drop" value="Drop"', confirm(), '>';}
	echo '<input type="hidden" name="token" value="', $T, '">
</form>
';} elseif (isset($_GET["procedure"])) {
	...
	$L = $_POST;
	$L["fields"] = (array) $L["fields"];if ($_POST && !process_fields($L["fields"]) && !$k) {
		$Xf = "$L[name]_adminer_" . uniqid();
		drop_create("DROP $if " . idf_escape($da), create_routine($if, $L), "DROP $if " . idf_escape($L["name"]), create_routine($if, array("name" => $Xf) + $L), "DROP $if " . idf_escape($Xf), substr(ME, 0, -1), 'Routine has been dropped.', 'Routine has been altered.', 'Routine has been created.', $da, $L["name"]);}
	page_header(($da != "" ? (isset($_GET["function"]) ? 'Alter function' : 'Alter procedure') . ": " . h($da) : (isset($_GET["function"]) ? 'Create function' : 'Create procedure')), $k);if (!$_POST && $da != "") {
		$L = routine($da, $if);
		$L["name"] = $da;}
...
<table cellspacing="0" class="nowrap">
';
	edit_fields($L["fields"], $Xa, $if);if (isset($_GET["function"])) {
...
<p>';
	...
	echo '<p>
<input type="submit" value="Save">
';if ($da != "") {echo '<input type="submit" name="drop" value="Drop"', confirm(), '>';}
	echo '<input type="hidden" name="token" value="', $T, '">
</form>
';} elseif (isset($_GET["trigger"])) {
	...
	$L = (array) trigger($F) + array("Trigger" => $a . "_bi");if ($_POST) {
		...
		$L = $_POST;}
	page_header(($F != "" ? 'Alter trigger' . ": " . h($F) : 'Create trigger'), $k, array("table" => $a));
	echo '
<form action="" method="post" id="form">
<table cellspacing="0">
...
';} elseif (isset($_GET["user"])) {
	$fa = $_GET["user"];
	$Le = array("" => array("All privileges" => ""));foreach (get_rows("SHOW PRIVILEGES") as $L) {
	...
	$Le["Databases"]["Create routine"] = $Le["Procedures"]["Create routine"];unset($Le["Procedures"]["Create routine"]);
	$Le["Columns"] = array();foreach (array("Select", "Insert", "Update", "References") as $X) {
	...
	$Sd = "";if (isset($_GET["host"]) && ($J = $e->query("SHOW GRANTS FOR " . q($fa) . "@" . q($_GET["host"])))) {
		while ($L = $J->fetch_row()) {
			if (preg_match('~GRANT (.*) ON (.*) TO ~', $L[0], $C) && preg_match_all('~ *([^(,]*[^ ,(])( *\\([^)]+\\))?~', $C[1], $rd, PREG_SET_ORDER)) {
				foreach ($rd as $X) {
					if ($X[1] != "USAGE") {
						$yc["$C[2]$X[2]"][$X[1]] = true;
					}
		...
		$Td = (isset($_GET["host"]) ? q($fa) . "@" . q($_GET["host"]) : "''");if ($_POST["drop"]) {
			...
			$Jd = q($_POST["user"]) . "@" . q($_POST["host"]);
			$ve = $_POST["pass"];if ($ve != '' && !$_POST["hashed"]) {
			...
			$lb = false;if (!$k) {
				...
				$ff = array();foreach ($Hd as $Nd => $r) {
					...
					$r = array_keys($r);if (isset($_GET["grant"])) {
						...
						$Qd = array_keys((array) $yc[$Nd]);
						$ff = array_diff($Qd, $r);
						$r = array_diff($r, $Qd);unset($yc[$Nd]);}if (preg_match('~^(.+)\\s*(\\(.*\\))?$~U', $Nd, $C) && (!grant("REVOKE", $ff, $C[2], " ON $C[1] FROM $Jd") || !grant("GRANT", $r, $C[2], " ON $C[1] TO $Jd"))) {
						$k = true;
						break;}}}if (!$k && isset($_GET["host"])) {
				if ($Td != $Jd) {
					queries("DROP USER $Td");
				} elseif (!isset($_GET["grant"])) {
					foreach ($yc as $Nd => $ff) {
						if (preg_match('~^(.+)(\\(.*\\))?$~U', $Nd, $C)) {
	...
	page_header((isset($_GET["host"]) ? 'Username' . ": " . h("$fa@$_GET[host]") : 'Create user'), $k, array("privileges" => array('', 'Privileges')));if ($_POST) {
		$L = $_POST;
		$yc = $Hd;} else {
...
';
	...
	echo "</table>\n", '<p>
<input type="submit" value="Save">
';if (isset($_GET["host"])) {echo '<input type="submit" name="drop" value="Drop"', confirm(), '>';}
	echo '<input type="hidden" name="token" value="', $T, '">
</form>
';} elseif (isset($_GET["processlist"])) {
	if (support("kill") && $_POST && !$k) {
		$cd = 0;foreach ((array) $_POST["kill"] as $X) {
...
<script type=\'text/javascript\'>tableCheck();</script>
<p>
';if (support("kill")) {echo ($t + 1) . "/" . sprintf('%d in total', $e->result("SELECT @@max_connections")), "<p><input type='submit' value='" . 'Kill' . "'>\n";}
	echo '<input type="hidden" name="token" value="', $T, '">
</form>
';} elseif (isset($_GET["select"])) {
	...
	$zc = ($s && $Vc ? "\nGROUP BY " . implode(", ", $s) : "") . ($de ? "\nORDER BY " . implode(", ", $de) : "");if ($_GET["val"] && is_ajax()) {
		...
		foreach ((array) $zg as $z => $X) {
			...
			dump_headers($a);
			$b->dumpTable($a, "");if (!is_array($_POST["check"]) || $zg === array()) {
				$I = "SELECT $wc$Pg$zc";
			} else {
				$vg = array();foreach ($_POST["check"] as $X) {
			...
			if ($_POST["save"] || $_POST["delete"]) {
				...
				$P = array();if (!$_POST["delete"]) {
					foreach ($d as $F => $X) {
						$X = process_input($m[$F]);if ($X !== null && ($_POST["clone"] || $X !== false)) {
					...
					if ($_POST["all"] || ($zg === array() && is_array($_POST["check"])) || $Vc) {
						...
						foreach ((array) $_POST["check"] as $X) {
							...
							$na += $e->affected_rows;}}}
				$D = lang(array('%d item has been affected.', '%d items have been affected.'), $na);if ($_POST["clone"] && $J && $na == 1) {
					$gd = last_id();if ($gd) {
				...
				if (!$_POST["val"]) {
					...
					$J = true;
					$na = 0;foreach ($_POST["val"] as $xg => $L) {
						$P = array();foreach ($L as $z => $X) {
							$z = bracket_escape($z, 1);
							$P[idf_escape($z)] = (preg_match('~char|text~', $m[$z]["type"]) || $X != "" ? $b->processInput($m[$z], $X) : "NULL");}
						$J = $j->update($a, $P, " WHERE " . ($Z ? implode(" AND ", $Z) . " AND " : "") . where_check($xg, $m), !($Vc || $zg === array()), " ");if (!$J) {
							break;
						}

						$na += $e->affected_rows;}
					queries_redirect(remove_from_uri(), lang(array('%d item has been affected.', '%d items have been affected.'), $na), $J);}} elseif (!is_string($mc = get_file("csv_file", true))) {
				$k = upload_error($mc);
	...
	$b->selectLinks($R, $P);if (!$d && support("table")) {
		...
		$b->selectOrderPrint($de, $d, $w);
		$b->selectLimitPrint($_);
		$b->selectLengthPrint($ag);
		$b->selectActionPrint($w);
		echo "</form>\n";
		$G = $_GET["page"];if ($G == "last") {
			$vc = $e->result(count_rows($a, $Z, $Vc, $s));
		...
		$J = $j->select($a, $pf, $Z, $s, $de, $_, $G, true);if (!$J) {
			...
			if (!$M) {
				...
				$Ba = $b->backwardKeys($a, $Pf);
				echo "<table id='table' cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);' onkeydown='return editingKeydown(event);'>\n", "<thead><tr>" . (!$s && $N ? "" : "<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='" . h($_GET["modify"] ? remove_from_uri("modify") : $_SERVER["REQUEST_URI"] . "&modify=1") . "'>" . 'Modify' . "</a>");
				$Gd = array();
				$xc = array();
				reset($N);
				$Te = 1;foreach ($M[0] as $z => $X) {
					if ($z != $Pd) {
						...
						$F = ($l ? $b->fieldName($l, $Te) : ($X["fun"] ? "*" : $z));if ($F != "") {
							...
							$xb = "&desc%5B0%5D=1";
							echo '<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">', '<a href="' . h($Hc . ($de[0] == $c || $de[0] == $z || (!$de && $Vc && $s[0] == $c) ? $xb : '')) . '">';
							echo
							apply_sql_function($X["fun"], $F) . "</a>";
							echo "<span class='column hidden'>", "<a href='" . h($Hc . $xb) . "' title='" . 'descending' . "' class='text'> â†“</a>";if (!$X["fun"]) {
				...
				foreach ($b->rowDescriptions($M, $o) as $E => $L) {
					...
					echo "<tr" . odd() . ">" . (!$s && $N ? "" : "<td>" . checkbox("check[]", substr($xg, 1), in_array(substr($xg, 1), (array) $_POST["check"]), "", "this.form['all'].checked = false; formUncheck('all-page');") . ($Vc || information_schema(DB) ? "" : " <a href='" . h(ME . "edit=" . urlencode($a) . $xg) . "'>" . 'edit' . "</a>"));foreach ($L as $z => $X) {
						if (isset($Gd[$z])) {
							...
							$X = select_value($X, $A, $l, $ag);
							$u = h("val[$xg][" . bracket_escape($z) . "]");
							$Y = $_POST["val"][$xg][bracket_escape($z)];
							$Mb = !is_array($L[$z]) && is_utf8($X) && $M[$E][$z] == $L[$z] && !$xc[$z];
							$Zf = preg_match('~text|lob~', $l["type"]);if (($_GET["modify"] && $Mb) || $Y !== null) {
								$Ac = h($Y !== null ? $Y : $L[$z]);
								echo "<td>" . ($Zf ? "<textarea name='$u' cols='30' rows='" . (substr_count($L[$z], "\n") + 1) . "'>$Ac</textarea>" : "<input name='$u' value='$Ac' size='$md[$z]'>");} else {
								$qd = strpos($X, "<i>...</i>");
								echo "<td id='$u' onclick=\"selectClick(this, event, " . ($qd ? 2 : ($Zf ? 1 : 0)) . ($Mb ? "" : ", '" . h('Use edit link to modify this value.') . "'") . ");\">$X";}}}if ($Ba) {
						echo "<td>";
					}

					$b->backwardKeysPrint($Ba, $M[$E]);
					echo "</tr>\n";}if (is_ajax()) {
					exit;
				}
				...
				$bc = true;if ($_GET["page"] != "last") {
					...
					$ud = ($vc === false ? $G + (count($M) >= $_ ? 2 : 1) : floor(($vc - 1) / $_));if ($y != "simpledb") {
						...
						if ($ud > 0) {echo ($G + 5 < $ud ? " ..." : ""), ($bc && $vc !== false ? pagination($ud, $G) : " <a href='" . h(remove_from_uri("page") . "&page=last") . "' title='~$ud'>" . 'last' . "</a>");}
						echo (($vc === false ? count($M) + 1 : $vc - $G * $_) > $_ ? ' <a href="' . h(remove_from_uri("page") . "&page=" . ($G + 1)) . '" onclick="return !selectLoadMore(this, ' . (+$_) . ', \'' . 'Loading' . '...\');" class="loadmore">' . 'Load more data' . '</a>' : '');} else {echo 'Page' . ":", pagination(0, $G) . ($G > 1 ? " ..." : ""), ($G ? pagination($G, $G) : ""), ($ud > $G ? pagination($G + 1, $G) . ($ud > $G + 1 ? " ..." : "") : "");}}
				echo "<p class='count'>\n", ($vc !== false ? "(" . ($bc ? "" : "~ ") . lang(array('%d row', '%d rows'), $vc) . ") " : "");
				$Bb = ($bc ? "" : "~ ") . $vc;
				echo
...
';}
				$tc = $b->dumpFormat();foreach ((array) $_GET["columns"] as $c) {
					...
					$me = $b->dumpOutput();
					echo ($me ? html_select("output", $me, $ma["output"]) . " " : ""), html_select("format", $tc, $ma["format"]), " <input type='submit' name='export' value='" . 'Export' . "'>\n", "</div></fieldset>\n";}
			...
			$b->selectEmailPrint(array_filter($Rb, 'strlen'), $d);
			echo "<p><input type='hidden' name='token' value='$T'></p>\n", "</form>\n";}}if (is_ajax()) {ob_end_clean();exit;}} elseif (isset($_GET["variables"])) {
	$Df = isset($_GET["status"]);
	page_header($Df ? 'Status' : 'Variables');
	$Ig = ($Df ? show_status() : show_variables());if (!$Ig) {
		echo "<p class='message'>" . 'No rows.' . "\n";
	} else {
	...
	header("Content-Type: text/javascript; charset=utf-8");if ($_GET["script"] == "db") {
		$Mf = array("Data_length" => 0, "Index_length" => 0, "Data_free" => 0);foreach (table_status() as $F => $R) {
			json_row("Comment-$F", nbsp($R["Comment"]));if (!is_view($R)) {
	...
	$Uf = array_merge((array) $_POST["tables"], (array) $_POST["views"]);if ($Uf && !$k && !$_POST["search"]) {
		...
		if ($_POST["truncate"]) {
			...
			$D = 'Tables have been truncated.';} elseif ($_POST["move"]) {
			$J = move_tables((array) $_POST["tables"], (array) $_POST["views"], $_POST["target"]);
			$D = 'Tables have been moved.';} elseif ($_POST["copy"]) {
			$J = copy_tables((array) $_POST["tables"], (array) $_POST["views"], $_POST["target"]);
			$D = 'Tables have been copied.';} elseif ($_POST["drop"]) {
			if ($_POST["views"]) {
			...
			$D = 'Tables have been dropped.';} elseif ($y != "sql") {
			$J = ($y == "sqlite" ? queries("VACUUM") : apply_queries("VACUUM" . ($_POST["optimize"] ? "" : " ANALYZE"), $_POST["tables"]));
			$D = 'Tables have been optimized.';} elseif (!$_POST["tables"]) {
			$D = 'No tables.';
	...
	page_header(($_GET["ns"] == "" ? 'Database' . ": " . h(DB) : 'Schema' . ": " . h($_GET["ns"])), $k, true);if ($b->homepage()) {
		if ($_GET["ns"] !== "") {
			...
			$Tf = tables_list();if (!$Tf) {
				...
				echo '<th>' . 'Table', '<td>' . 'Engine' . doc_link(array('sql' => 'storage-engines.html')), '<td>' . 'Collation' . doc_link(array('sql' => 'charset-mysql.html')), '<td>' . 'Data Length' . $Cb, '<td>' . 'Index Length' . $Cb, '<td>' . 'Data Free' . $Cb, '<td>' . 'Auto Increment' . doc_link(array('sql' => 'example-auto-increment.html')), '<td>' . 'Rows' . $Cb, (support("comment") ? '<td>' . 'Comment' . $Cb : ''), "</thead>\n";
				...
				echo "<tr><td>&nbsp;<th>" . sprintf('%d in total', count($Tf)), "<td>" . nbsp($y == "sql" ? $e->result("SELECT @@storage_engine") : ""), "<td>" . nbsp(db_collation(DB, collations()));foreach (array("Data_length", "Index_length", "Data_free") as $z) {
					echo "<td align='right' id='sum-$z'>&nbsp;";
				}

				echo "</table>\n";if (!information_schema(DB)) {
					$Gg = "<input type='submit' value='" . 'Vacuum' . "'" . on_help("'VACUUM'") . "> ";
					$ae = "<input type='submit' name='optimize' value='" . 'Optimize' . "'" . on_help($y == "sql" ? "'OPTIMIZE TABLE'" : "'VACUUM OPTIMIZE'") . "> ";
					echo "<fieldset><legend>" . 'Selected' . " <span id='selected'></span></legend><div>" . ($y == "sqlite" ? $Gg : ($y == "pgsql" ? $Gg . $ae : ($y == "sql" ? "<input type='submit' value='" . 'Analyze' . "'" . on_help("'ANALYZE TABLE'") . "> " . $ae . "<input type='submit' name='check' value='" . 'Check' . "'" . on_help("'CHECK TABLE'") . "> " . "<input type='submit' name='repair' value='" . 'Repair' . "'" . on_help("'REPAIR TABLE'") . "> " : ""))) . "<input type='submit' name='truncate' value='" . 'Truncate' . "'" . confirm() . on_help($y == "sqlite" ? "'DELETE'" : "'TRUNCATE" . ($y == "pgsql" ? "'" : " TABLE'")) . "> " . "<input type='submit' name='drop' value='" . 'Drop' . "'" . confirm() . on_help("'DROP TABLE'") . ">\n";
					$h = (support("scheme") ? $b->schemas() : $b->databases());if (count($h) != 1 && $y != "sqlite") {
			...
			echo '<p class="links"><a href="' . h(ME) . 'create=">' . 'Create table' . "</a>\n", (support("view") ? '<a href="' . h(ME) . 'view=">' . 'Create view' . "</a>\n" : ""), (support("materializedview") ? '<a href="' . h(ME) . 'view=&amp;materialized=1">' . 'Create materialized view' . "</a>\n" : "");if (support("routine")) {
				...
				$kf = routines();if ($kf) {
					...
					odd('');foreach ($kf as $L) {echo '<tr' . odd() . '>', '<th><a href="' . h(ME) . ($L["ROUTINE_TYPE"] != "PROCEDURE" ? 'callf=' : 'call=') . urlencode($L["ROUTINE_NAME"]) . '">' . h($L["ROUTINE_NAME"]) . '</a>', '<td>' . h($L["ROUTINE_TYPE"]), '<td>' . h($L["DTD_IDENTIFIER"]), '<td><a href="' . h(ME) . ($L["ROUTINE_TYPE"] != "PROCEDURE" ? 'function=' : 'procedure=') . urlencode($L["ROUTINE_NAME"]) . '">' . 'Alter' . "</a>";}
					echo "</table>\n";}
				echo '<p class="links">' . (support("procedure") ? '<a href="' . h(ME) . 'procedure=">' . 'Create procedure' . '</a>' : '') . '<a href="' . h(ME) . 'function=">' . 'Create function' . "</a>\n";}if (support("event")) {
				echo "<h3 id='events'>" . 'Events' . "</h3>\n";
				$M = get_rows("SHOW EVENTS");if ($M) {
					echo "<table cellspacing='0'>\n", "<thead><tr><th>" . 'Name' . "<td>" . 'Schedule' . "<td>" . 'Start' . "<td>" . 'End' . "<td></thead>\n";foreach ($M as $L) {echo "<tr>", "<th>" . h($L["Name"]), "<td>" . ($L["Execute at"] ? 'At given time' . "<td>" . $L["Execute at"] : 'Every' . " " . $L["Interval value"] . " " . $L["Interval field"] . "<td>$L[Starts]"), "<td>$L[Ends]", '<td><a href="' . h(ME) . 'event=' . urlencode($L["Name"]) . '">' . 'Alter' . '</a>';}
					echo "</table>\n";
