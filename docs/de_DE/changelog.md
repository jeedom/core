...
- **Szenario** : Unterstützung für generische Typen in Szenarien hinzugefügt.
	- Abzug : ``#genericType(LIGHT_STATE,#[Wohnzimmer]#)# > 0`
	- IF `genericType (LIGHT_STATE .),#[Wohnzimmer]#) > 0`
	- `GenericType`-Aktion
...
- **Aufbau** : Tab ** ist jetzt in der Registerkarte *Allgemein*.
- **Aufbau** : Tab ** ist jetzt in der Registerkarte *Ausrüstung*.
...
-  : Hinzufügen eines benutzerdefinierten CSS-Felds zu den Elementen des *Entwurf*.
