...
Hier erstellen Sie Ihr Szenario. Nach dem Erstellen des Szenarios ichst sein Inhalt leer, sodass er ... nichts tut. Du musst mit anfangen **Block hinzufügen**, mit dem Knopf rechts. Sobald ein Block erstellt wurde, können Sie einen weiteren hinzufügen **bloc** oder a **action**.
...
- $ szenario-> setLog ($ Nachricht); ` : Schreiben Sie eine Nachricht ichn das Skriptprotokoll.
...
- **Färbung von Dashboard-Symbolen** (SetColoredIcon) : Ermöglicht das Bisktivieren oder Nicht-Aktivieren der Farbgebung von Symbolen ichm Dashboard.
