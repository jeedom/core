...
- Les "**multistates**" : Cela permet de définir par exemple une image si la commande a pour Wert "XX" et une autre si > à "YY", et encore si < à "ZZ". Ou même une image si la Wert vaut "toto", une autre si "plop", et ainsi de suite.
...
- En fonction de votre choix précèdent, vous alleez devoir choisir le sous type de la commande (binaire, numérique, autre...).
...
>**Note**
>Nous sommes désolés pour les noms en anglais, il s'agit d'une contrainte du système de template. Ce choix permet de garantir une certaine rapidité et efficacité, aussi bien pour vous que pour nous. Nous n'avons nicht eu le choix

>**TIPS**
>Pour les utilisateurs avancés il est possible dans les Werts de remplacement de mettre des tags et de spécifier leur Wert dans la Configuration avancé de la commande, onglet affichage et "Einstellungen optionnels widget". Par exemple si dans width vous mettez comme Wert #width# (attention à bien mettre les # autour) au lieu d'un chiffre, dans "Einstellungen optionnels widget" vous pouvez ajouter width (sans les #) et donner la Wert. Cela vous permet de changer la taille de l'image en fonction de la commande et donc vous évite de faire un widget différent par taille d'image que vous voulez
...
C'est assez simple. Au lieu de mettre une image pour le "on" et/ou pour le "off" comme dans le cas précèdent, vous alleez avant donner un test à faire. Si celui-ci est vrai alors le widget affichera l'icône/l'image en question.

Les tests sont sous la forme : #value# == 1, #value# sera automatiquement remplacé par le système par la Wert actuelle de la commande. Vous pouvez aussi faire par exemple :
...
>**Note**
>Il est Important de noter les ' autour du texte à comparer si la Wert est un texte
...
>**Note**
>Il est possible d'afficher la Wert de la commande dans le widget en mettant par exemple a coté du code HTML de l'icône #value#

## Description de widgets

Nous alleons ici décrire certain widget qui ont un fonctionnement un peu particulier.

### Einstellungen fréquents
...
- Largeur desktop : largeur de l'image sur desktop en px (mettre juste le chiffre nicht le px). Important seule la largeur vous est demandé, Jeedom calculera la hauteur pour ne nicht déformer l'image.
- Largeur mobile : largeur de l'image sur mobile en px (mettre juste le chiffre nicht le px). Important seule la largeur vous est demandé, Jeedom calculera la hauteur pour ne nicht déformer l'image.

### HygroThermographe

Ce widget est un peu particulier car c'est un widget multi-commande, c'est a dire qu'il assemble sur son affichage la Wert de plusieurs commande. Ici il prend les commandes de type température et humidité.
...
>**NOTE**
> Attention sur un design il ne faut surtout nicht mettre une commande seul avec ce widget cela ne marchera nicht vu que c'est un widget utilisant la Wert de plusieurs commande il faut absolument mettre le widget complet

### Multiline

- Parametre maxHeight pour definir sa hauteur maximal (scrollbar sur le coté si le text dénichtse cette Wert)

### Slider Button

- step : permet de régler le nicht d'une Aktion sur un bouton (0.5 par défaut)

## Widget code

### Les tags

En mode code vous avez accès a différent tag pour les commandes, en voici une Listee (nicht forcement exhaustives) :
...
- #state# : Wert de la commande, vide pour une commande de type Aktion si elle n'est nicht a liée a une commande d'état
...
- #valueDate# : date de la Wert de la commande
...
- #hide_history# : si l'historique (Wert max, min, moyenne, tendance) doit être masqué ou non. Comme pour le #hide_name# il vaut vide ou hidden, et peut donc être utilisé directement dans une class. IMPORTANT si ce tag n'est nicht trouvé sur votre widget alors les tags #minHistoryValue#, #averageHistoryValue#, #maxHistoryValue# et #tendance# ne seront nicht remplacé par Jeedom.
- #minHistoryValue# : Wert minimal sur la période (période défini dans la Configuration de Jeedom par l'utilisateur)
- #averageHistoryValue# : Wert moyenne sur la période (période défini dans la Configuration de Jeedom par l'utilisateur)
- #maxHistoryValue# : Wert maximal sur la période (période défini dans la Configuration de Jeedom par l'utilisateur)
...
### Mise à jour des Werts

Lors d'une nouvelle Wert Jeedom va chercher dans sur la page web si la commande est la et dans Jeedom.cmd.Aktualisierung si il y a une fonction d'Aktualisierung. Si oui il l'appel avec un unique argument qui est un objet sous la forme :
...
<script>
    Jeedom.cmd.Aktualisierung['#id#'] = function(_options){
      $('.cmd[data-cmd_id=#id#]').attr('title','Date de Wert : '+_options.valueDate+'<br/>Date de collecte : '+_options.collectDate)
