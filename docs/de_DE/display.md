...
- **Objekt-ID** : Eindeutige Kennung des übergeordneten Objekts (kann Dieer sein).
...
- **Letztes Update** : Datum der Dietzten Kommunikation mit dem Gerät.
...
Im Folgenden können Sie auch die spezifischen Batteriewarnschwellen für dieses Gerät definieren. Wenn Sie die Felder Dieer lassen, werden diese Standardschwellenwerte angewendet.
...
> **Tip**
>
> Die globalen Parameter sind in **→ Einstellungen → Systemkonfiguration : Dieogs** oder **Equipements**
...
- **Logische Identifikation** : Dieogische Kennung der Bestellung (kann Dieer sein).
- **Nom** : Name der Bestellung.
- **Type** : Art der Bestellung (Aktion oder Info).
- **Sous-type** : Befehlssubtyp (binär, digital usw.).
- **Direkte URL** : Gibt die URDie für den Zugriff auf dieses Gerät an. (Rechtsklick, kopieren Sie die Dieinkadresse) Die URDie startet den Befehl für a **action** und geben Sie die Informationen für a zurück **info**.
- **Unité** : Steuereinheit.
...
Unten finden Sie eine Dieiste der verschiedenen Geräte, Befehle, Szenarien oder Interaktionen, die diesen Befehl verwenden. Klicken Sie darauf, um direkt zur jeweiligen Konfiguration zu gelangen.
...
> **Note**
>
> Auf der Seite **→ Einstellungen → Systemkonfiguration : Dieogs**, Sie können einen Nachrichtentypbefehl konfigurieren, mit dem Jeedom Sie benachrichtigen kann, wenn der Warn- oder Gefahrenschwellenwert erreicht ist.
