...
- **** : O modal de histórico e a página de histórico permitem usar botões *Semana, Mês, Ano* para recarregar dinamicamente um histórico maior.
- **** : Um atraso de 0.25s foi introduzido na abertura de submenus.
...
- **** : Capacidade de exibir vários objetos lado a lado *(Configurações → Sistema → Configuração / Interface)*.
- **** : A janela de edição do bloco Edit Mode permite que os comandos sejam renomeados.
- **** : No layout da tabela, possibilidade de inserir atributos HTML *(colspan/rowspan em particular)* para cada célula.
- **** : Capacidade de desativar os modelos de widget de plug-ins que os utilizam para retornar à exibição padrão do Jeedom *(janela de configuração do dispositivo)*.
- **** : O equipamento tornado inativo desaparece automaticamente de todas as páginas. O equipamento reativado reaparece no painel se o objeto pai já estiver presente.
- **** : Equipamentos tornados invisíveis desaparecem automaticamente do painel. O equipamento reexibido reaparece no painel se o objeto pai já estiver presente.
...
- **** : Excluir ou criar um resumo resulta na atualização do resumo global e do assunto.
- **Ferramentas > Substituir** : Esta ferramenta agora oferece um modo **, permitindo copiar as configurações de equipamentos e comandos, sem substituí-los nos cenários e outros.
- **** : A linha do tempo agora carrega os primeiros 35 eventos. Na parte inferior da página, vários botões permitem que você carregue dinamicamente os seguintes eventos.
- **** : Possibilidade de diferenciar ações em caso de erro ou alerta de comando.
- **** : Capacidade de definir widgets de comando padrão.
- Um menu contextual foi adicionado em diferentes lugares ao nível das caixas de seleção para selecionar todas, nenhuma ou inverter a seleção *(consulte [Desenvolvedor de documentos](https:doc.jeedom.com/pt_PT/devcore4.4))*.
- **** : possibilidade na página de configuração do objeto para pedir jeedom para reordenar o equipamento de acordo com seu uso

### 4.4 : Autre

- **** : Início do desenvolvimento em js puro, sem jQuery. Ver [Desenvolvedor de documentos](https:doc.jeedom.com/pt_PT/devcore4.4).
- **** : Listagem mais detalhada de dispositivos USB.
- **** : Atualizar Highchart v9.3.2 a v10.3.2 (O módulo ** não é mais importado).

### 4.4 : Remarques

> ****
>
> No **** e a ****, Núcleo v4.4 agora redimensiona automaticamente os blocos para criar uma grade perfeita. As unidades (menor altura e menor largura de um ladrilho) desta grade são definidas em **Configurações → Sistema → Configuração / Interface** por valores *Passo vertical (mínimo 100)*  *Passo horizontal (mínimo 110)*. O valor que ** definindo o espaço entre as telhas.
> Os ladrilhos se adaptam às dimensões da grade e podem ser feitos uma, duas vezes etc. esses valores em altura ou largura. Certamente será necessário passar [Modo de edição do painel](https:doc.jeedom.com/pt_PT/core4.4dashboard#Mode%20%C3%A9dition) para ajustar o tamanho de alguns blocos após a atualização.
...
> Todos os widgets Core agora suportam a exibição **, adicionando um parâmetro opcional **  **  **.

> **Caixa de diálogo**
>
> Todas as caixas de diálogo (bootstrap, bootbox, jQuery UI) foram migradas para uma biblioteca interna Core especialmente desenvolvida. Caixas de diálogo redimensionáveis agora têm um botão para alternar **.
...
- Autorização de resposta livre em ** se você colocar * no campo de respostas possíveis.
...
- Descrição adicionada **** para ações de mensagem (consulte [Desenvolvedor de documentos](https:doc.jeedom.com/pt_PT/devcore4.3)).
...
- Adicionando uma ação ** para cenários, ações de valor, ações pré/pós exec : Permite alterar o tema da interface imediatamente, no escuro, claro ou outro (alternar).
...
- Corrigido erro de inclusão com alguns plugins (** por exemplo).
...
- **** : Adicionado um botão para copiar os direitos de um usuário limitado para outro.
...
- **** : Possibilidade de desabilitar a instalação de dependências por plugin.
- **** : jeeFrontEnd{}, jeephp2js{}, pequenas correções de bugs e otimizações.
...
- **** : Atualizar fonte impressionante 5.13.1 a 5.15.4.
...
``<a onClick="jeephp2js.planHeader_id=15; jeeFrontEnd.plan.displayPlan();"><li class="monmenu"><div class="imagette"><img src="theme1imagesnewhome.png" height=30px><div><br><li><a>``
 ...
 [Desenvolvedor de documentos](https:doc.jeedom.com/pt_PT/devcore4.3).

Postagem do blog [](https:blog.jeedom.com6739-jeedom-4-3)
...
-  : Link para documentação quando o plug-in estiver em beta.
...
-  : Maior atraso para entrar em contato com o serviço de monitoramento (permite aliviar a carga nos servidores em nuvem).

## 

- IU de correção de bug : Cenas : Adicionando a ação ** no modo de seleção.
- Núcleo de correção de bugs : Corrigido o atraso nos históricos calculados.
-  : Instalação de dependências de plugin zigbee.
...
- IU de correção de bug :  *Ocultar na área de trabalho* resumos.
...
- Núcleo de correção de bugs : Adicionar opção ** em um gráfico em Design.
...
- Núcleo de correção de bugs : Atualizar valor ao clicar no intervalo de widgets **.
...
- Núcleo de correção de bugs : A atualização da página indica *Em andamento*  *ERRO DE ATUALIZAÇÃO* (atualização de log).
...
- IU de correção de bug :  *Não exibir mais* no modal *primeiro usuário*.
...
- **Síntese** : Possibilidade de configurar objetos para ir a um ** ou um ** desde a síntese.
- **** : A janela de configuração do dispositivo (modo de edição) agora permite que você configure widgets móveis e tipos genéricos.
- **** : Internacionalização de Widgets de terceiros (código do usuário). Vejo [Desenvolvedor de documentos](https:doc.jeedom.com/pt_PT/devcore4.2).
...
- **** : Coloração de logs de acordo com certos eventos. Possibilidade de exibir o log bruto.
...
- **Resumos** : Agora é possível configurar [ações em resumos](/pt_PT/conceptsummary#Actions  résumés) (ctrl + clique em um resumo) graças aos virtuais.
- **** : Visualizar arquivos PDF.
- **Tipos de equipamento** : [Nova página](/pt_PT/core4.2types) **Ferramentas → Tipos de equipamento** permitindo que tipos genéricos sejam atribuídos a dispositivos e comandos, com suporte para tipos dedicados a plug-ins instalados (ver [Desenvolvedor de documentos](https:doc.jeedom.com/pt_PT/devcore4.2)).
- **Seleção de ilustrações** : Nova janela global para a escolha de ilustrações *(ícones, imagens, planos de fundo)*.
- **Exibir mesa** : Adição de um botão à direita da pesquisa nas páginas ** *Cenários* ** **  ** para mudar para o modo de mesa. Isso é armazenado por um cookie ou em **Configurações → Sistema → Configuração / Interface, Opções**. Os plugins podem usar esta nova função do Core. Vejo [Desenvolvedor de documentos](https:doc.jeedom.com/pt_PT/devcore4.2).
...
- **** : Capacidade de gerenciar os perfis de diferentes usuários Jeedom a partir da página de gerenciamento de usuários.
- **** : Capacidade de ocultar objetos / visualização / design / design 3D para usuários limitados.
...
- ****: Capacidade de desabilitar o monitoramento de nuvem (Administração / Atualizações / Mercado).
...
- Widgets principais com exibição de um ** agora suporta um parâmetro opcional ** : ** para exibir uma data relativa (ontem às 16h48, segunda-feira passada às 14h, etc).
- Widgets do tipo cursor (ação) agora aceitam um parâmetro opcional ** para definir a etapa de mudança no cursor.
- O widget **ação.slider.value** agora está disponível no desktop, com um parâmetro opcional **, o que o torna um ** .
- O widget **** (**) foi refeito em puro css e integrado em dispositivos móveis. Eles são, portanto, agora idênticos em computadores e dispositivos móveis.
...
Os plugins devem respeitar as recomendações sobre a estrutura em árvore de pastas e arquivos : [](https:doc.jeedom.com/pt_PT/devplugin_template).

[Blog: Jeedom 4 introdução.2 : Segurança](https:blog.jeedom.com6165-introduction-jeedom-4-2-la-securite)
...
- Revisão da opção de configuração do comando **Gerenciando a repetição de valores** quem se torna **Repita valores idênticos (Sim|Non)**. [Veja o artigo do blog para mais detalhes](https:blog.jeedom.com5414-nouvelle-gestion-de-la-repetition-des-valeurs)
...
- Bugfix de elementos ** em um design.
...
- **** : O tamanho do bloco de um novo equipamento se adapta ao seu conteúdo.
- **** : Adicionar (voltar!) Um botão para filtrar os itens exibidos por categoria.
- **** : Ctrl Clique em uma informação para abrir a janela do histórico com todos os comandos históricos do equipamento visíveis no bloco. Ctrl Clique em uma legenda para exibir apenas esta, Alt Clique para exibir todas.
- **** : Redesenho da exibição da árvore de objetos (seta à esquerda da pesquisa).
- **** : Capacidade de desfocar imagens de fundo (Configuração -> Interface).
...
- **** : Adicionando um widget principal **.
- **** : Adicionando um widget principal **.
- **Update Center** : As atualizações são verificadas automaticamente quando a página é aberta, se for 120 minutos mais antiga.
- **Update Center** : A barra de progresso está agora na guia *Núcleo e plugins*, e o log aberto por padrão na guia **.
...
- **Cenas** : Correção de bug de ** em bloco copiar / colar.
...
- **Análise / História** : Agora podemos usar a opção ** com a opção **.
- **Análise / Logs** : Nova fonte de tipo monoespaçado para logs.
- **** : Possibilidade de colocar cenários.
- **** : Modo de edição agora inserindo o bloco movido.
- **** : Modo de edição: os ícones de atualização do equipamento são substituídos por um ícone que permite acesso à sua configuração, graças a um novo modal simplificado.
...
- **** : Possibilidade de adicionar css de classe a um bloco (consulte a documentação do widget).
...
- **** :  adição de uma opção para proibir uma chave de API de um plugin de executar métodos centrais (geral)
...
### 4.1 : Changements
- A função **cenário-> getHumanName()** da classe de cenário php não retorna mais *[objeto] [grupo] [nome]*  *[grupo] [objeto] [nome]*.
...
- Durante uma pesquisa, a chave ** cancelar pesquisa.
-  : No modo de edição, o controle de pesquisa e seus botões são desativados e se tornam fixos.
-  : No modo de edição, um clique de um botão ** à direita dos objetos redimensiona os ladrilhos do objeto para a altura do mais alto. Ctrl + clique os reduz à altura do mais baixo.
-  : A execução do comando em um bloco agora é sinalizada pelo botão **. Se não houver nenhum no bloco, ele aparecerá durante a execução.
-  : Os blocos indicam um comando de informação (historizado, que abrirá a janela Histórico) ou ação ao passar o mouse.
-  : A janela de histórico agora permite que você abra este histórico em Análise / Histórico.
-  : A janela de histórico mantém sua posição / dimensões ao reabrir outro histórico.
...
-  : Adicionada opção para usar cores personalizadas.
-  : Adição de um menu contextual nas guias (mudança rápida de objeto).
-  : Adição de um menu contextual nas guias (mudança rápida de interação).
-  : Adição de um menu contextual nas abas (troca rápida de equipamento).
-  : Na página de gerenciamento de plug-ins, um ponto laranja indica que os plug-ins em versão não estável.
...
-  : Redesenho do sistema de widgets (menu Ferramentas / Widgets).
-  : Possibilidade de substituir um widget por outro em todos os comandos que o utilizam.
-  : Possibilidade de atribuir widgets a vários comandos.
-  : Adicionar um widget de informação numérica horizontal.
-  : Adicionar um widget de informação numérica vertical.
...
- Cenas : Adição de uma função de pesquisa em blocos de código.  : Ctrl + F, em seguida, Enter, próximo resultado : Ctrl + G, resultado anterior : Ctrl+Shift+G
...
- **** : Atualize o jquery 3.4.1
- **** : Atualizar CodeMiror 5.46.0
- **** : Atualizar o tablesorter 2.31.1
...
- O plugin do widget não é compatível com esta versão do Jeedom e não será mais suportado (porque os recursos foram levados internamente no núcleo). Mais Informações [](https:www.Jeedom.comblog4368-les-widgets-en-v4).
