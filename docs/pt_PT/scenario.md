...
-  ``cmd::byString($string);`` : Retorna o objeto de comando correspondente.
    -   ``$string``: Link para o pedido desejado : ``#[objet][][commande]#`` ( : ``#[Appartement][Alarme][De ativos]#``)
...
- ``matches`` : . Ex : ``[Salle de bain][Hydrometrie][etat] matches "humide"``.
...
- ``#sday#`` : Nome do dia da semana. Ex : .
- ``#nday#`` : Número do dia de 0 (domingo) a 6 (sábado).
- ``#smonth#`` : Nome do mês. Ex : .
