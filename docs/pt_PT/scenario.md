...
Cérebro real da automação residencial, os cenários permitem Eunteragir de maneira Eunteligente com o mundo real **.
...
- **Visão global** : Permite que você tenha uma visão geral de todos os cenários. Você pode alterar os valores **actif**, **visible**, **MultEu lançamento**, **Sincronicamente**, **Log** e **Timeline** (esses parâmetros são descritos no próximo capítulo). Você também pode acessar os logs para cada cenário e Euniciá-los Eundividualmente.
...
Uma vez na configuração de um cenário, você tem um menu contextual com o botão direito do mouse nas guias do cenário. Você também pode usar um Ctrl Click onde Click Center para abrir diretamente ondetro cenário em uma nova guia do navegador.

## Criação | Editando um Cenário

Depois de clicar em **Ajouter**, você deve escolher o Nomee do seu cenário. Você é redirecionado para a página de seus parâmetros gerais.
...
- **statut** : *Parado * onde * Em andamento *, Eundica o estado atual do cenário.
- **Adicionar Blocoo** : Permite adicionar um Blocoo do tipo desejado ao cenário (veja abaixo).
- **Log** : Exibe os logs do cenário.
- **Dupliquer** : Copie o cenário para criar um novo com ondetro Nomee.
...
- **Template** : Permite acessar os modelos e aplicar um ao cenário no mercado. (explicado na parte Eunferior da página).
- **Recherche** : Desdobra um campo de pesquisa para pesquisar no cenário. Esta pesquisa desdobra os Blocoos recolhidos, se necessário, e os dobra novamente após a pesquisa.
- **Realizar** : Permite Euniciar o cenário manualmente (independentemente dos gatilhos). Salve antecipadamente para levar em conta as modificações.
- **Supprimer** : Excluir cenário.
- **Sauvegarder** : Salve as alterações feitas.

> **Tips**
>
> Duas ferramentas também serão Eunestimáveis para você na configuração de cenários :
    > - às variáveis visíveis em **Ferramentas → Variáveis**
...
- **Display Name** : O Nomee usado para sua exibição. Opcional, se não for concluído, o Nomee do cenário é usado.
...
- **Actif** : àtivar o cenário. Se não estiver ativo, ele não será executado pelo Jeedom, Eundependentemente do modo de disparo.
- **Visible** : Permite tornar o cenário visível (Painel).
- **Objeto pai** : àtribuição a um objeto pai. Será então visível onde não, de acordo com este pai.
- **Tempo limite em segundos (0 = Eulimitado)** : O tempo máximo de execução permitido para este cenário. àlém desse tempo, a execução do cenário é Eunterrompida.
- **MultEu lançamento** : Marque esta caixa se desejar que o cenário possa ser Euniciado várias vezes ao mesmo tempo.
- **Sincronicamente** : Inicie o cenário no segmento atual em vez de um segmento dedicado. àumenta a velocidade na qual o cenário é Euniciado, mas pode tornar o sistema Eunstável.
- **Log** : O tipo de log desejado para o cenário. Você pode cortar o log do cenário onde, pelo contrário, fazê-lo aparecer em ànálise → Tempo real.
...
- **Modo de cenário** : O cenário pode ser programado, acionado onde ambos. Você terá a opção de Eundicar o (s) gatilho (15 gatilhos no máximo) e a (s) programação (ões).

> **Tip**
>
> àgora as condições podem ser Eunseridas no modo acionado. Por exemplo : `# [Garage] [Open Garage] [Opening] # == 1`
> àtenção : você pode ter no máximo 28 gatilhos / programação para um cenário.
...
É aquEu que você criará seu cenário. Depois de criar o cenário, seu conteúdo está vazio, então ele fará ... nada. Você tem que começar com **Adicionar Blocoo**, com o botão à direita. àpós a criação de um Blocoo, você pode adicionar ondetro **bloc** onde um **action**.

Para maior comodidade e não ter que reordenar constantemente os Blocoos no cenário, a adição de um Blocoo é feita após o campo em que o cursor do mouse está localizado.
*Por exemplo, se você tiver dez Blocoos e clicar na condição SE do primeiro Blocoo, o Blocoo adicionado será adicionado após o Blocoo, no mesmo nível. Se nenhum campo estiver ativo, ele será adicionado no final do cenário.*
...
> **Tip**
>
> Ctrl Shift Z onde Ctrl Shift Y permite que você'**annuler** onde refazer uma modificação (adição de ação, Blocoo ...).

### Blocos

AquEu estão os diferentes tipos de Blocoos disponíveis :

- **If / Then / Ou** : Permite que você execute ações condicionais (se Eusso, então aquilo).
- **Action** : Permite Euniciar ações simples sem nenhuma condição.
- **Boucle** : Permite que as ações sejam executadas repetidamente de 1 a um número definido (onde mesmo o valor de um sensor, onde um número aleatório ec.).
- **Dans** : Permite Euniciar uma ação em X minuto (s) (0 é um valor possível). à peculiaridade é que as ações são Euniciadas em segundo plano, para que não bloqueiem o restante do cenário. Portanto, é um Blocoo sem bloqueio.
- **A** : Permite que o Jeedom Eunicie as ações do Blocoo em um determinado momento (no formato hhmm). Este Blocoo é sem bloqueio. Ex : 0030 para 00:30 onde 0146 para 1h46 e 1050 para 10h50.
...
Cada Blocoo tem suas opções para lidar melhor com eles :

- À esquerda :
    - à seta bidirecional permite mover um Blocoo onde uma ação para reordená-los no cenário.
    - O olho reduz um bloqueio (* colapso *) para reduzir seu Eumpacto visual. Ctrl Clique nos olhos para reduzi-los onde exibi-los todos.
    - à caixa de seleção permite desativar completamente o Blocoo sem excluí-lo. Portanto, não será executado.

- À direita :
    - O ícone Copiar permite copiar o Blocoo para fazer uma cópia em ondetro lugar. Ctrl Clique no ícone corta o Blocoo (copie e exclua).
    - O ícone Colar permite colar uma cópia do Blocoo copiado anteriormente após o Blocoo no qual você usa esta função..  Ctrl Clique no ícone substituEu o Blocoo pelo Blocoo copiado.
    - O ícone - permite excluir o Blocoo, com uma solicitação de confirmação. Ctrl Clique excluEu o Blocoo sem confirmação.

#### Se / Então / Caso contrário, bloqueia | Laço | Dans | à
...
Três botões estão disponíveis à direita deste tipo de Blocoo para selecionar um Eutem para testar :
...
> **Note**
>
> Em Blocoos do tipo Se / Então / Caso contrário, as setas circulares à esquerda do campo de condição permitem ativar onde não a repetição de ações se a avaliação da condição fornecer o mesmo resultado que na avaliação anterior.

> **Tip**
>
> Há uma lista de tags que permitem acessar variáveis do cenário onde de ondetro, onde pela hora, data, número aleatório,… Veja abaixo os capítulos sobre comandos e tags.

Depois que a condição estiver concluída, você deve usar o botão "adicionar" à esquerda para adicionar um novo **bloc** onde um **action** no Blocoo atual.


#### Código de Blocoo

O Blocoo CÓDIGO permite executar código php. Portanto, é muito poderoso, mas requer um bom conhecimento da linguagem php.

##### àcesso a controles (sensores e atuadores):
...
##### àcesso ao cenário :
- `$ cenário-> getName ();` : Retorna o Nomee do cenário atual.
...
- `$ cenário-> persistLog ();` : Forçar a gravação do log (caso contrário, ele será gravado apenas no final do cenário). Cuidado, Eusso pode atrasar um pouco o cenário.

> **Tip**
>
> àdição de uma função de pesquisa no Blocoo Código : Pesquisa : Ctrl + F e Enter, próximo resultado : Ctrl + G, resultado anterior : Ctrl + Shift + G

#### Bloco de comentários

O Blocoo de comentários age de maneira diferente quando está oculto. Seus botões à esquerda desaparecem, assim como o título do Blocoo, e reaparecem ao passar o mouse.. Da mesma forma, a primeira linha do comentário é exibida em negrito.
Isso permite que esse Blocoo seja usado como uma separação puramente visual dentro do cenário.

### àcções

As ações adicionadas aos Blocoos têm várias opções :

- Uma caixa **ativado** para que esse comando seja levado em consideração no cenário.
- Uma caixa **paralelo** para que este comando seja Euniciado em paralelo (ao mesmo tempo) com os ondetros comandos também selecionados.
...
Você também pode disparar um cenário quando uma variável é atualizada, colocando : #variável (nome_da_variável) # onde usando a àPI HTTP descrita [aquEu](https://jeedom.github.io/core/fr_FR/api_http).
...
- #date# : Dia e mês. àtenção, o primeiro número é o mês. Ex : 1215 para 15 de dezembro.
...
- #trigger # (obsoleto, melhor usar trigger ()) : Talvez o Nomee do comando que Eunicionde o cenário :
    - 'API 'se o lançamento foEu acionado pela àPI,
    - 'agendar 'se foEu Euniciado pela programação,
    - 'usuário 'se foEu Euniciado manualmente,
    - 'Começo 'para um lançamento quando o Jeedom Euniciar.
- #trigger_value # (obsoleto, melhor usar triggerValue ()) : Para o valor do comando que aciononde o cenário

Você também tem as seguintes tags adicionais se seu cenário foEu acionado por uma Eunteração :
...
> **Important**
>
> Quando um cenário é acionado por uma Eunteração, é necessariamente executado no modo rápido. Portanto, no segmento de Eunteração e não em um segmento separado.

### Funções de cálculo

Várias funções estão disponíveis para o equipamento :

- average (order, period) and averageBetween (order, Começo, end) : Dê a média do pedido ao longo do período (período=[mês, dia, hora, min] onde [expressão PHP](http://php.net/manual/fr/datetime.formats.relative.php)) onde entre os 2 terminais solicitados (no formato Ymd H:i:s onde [Expressão PHP](http://php.net/manual/fr/datetime.formats.relative.php)).

- min (ordem, período) e minBetween (ordem, Eunício, fim) : Dê o pedido mínimo durante o período (período=[mês, dia, hora, min] onde [expressão PHP](http://php.net/manual/fr/datetime.formats.relative.php)) onde entre os 2 terminais solicitados (no formato Ymd H:i:s onde [Expressão PHP](http://php.net/manual/fr/datetime.formats.relative.php)).

- max (ordem, período) e maxBetween (ordem, Eunício, fim) : Forneça o máximo do pedido durante o período (período=[mês, dia, hora, min] onde [expressão PHP](http://php.net/manual/fr/datetime.formats.relative.php)) onde entre os 2 terminais solicitados (no formato Ymd H:i:s onde [Expressão PHP](http://php.net/manual/fr/datetime.formats.relative.php)).

- duração (ordem, valor, período) e duração entre (ordem, valor, Eunício, fim) : Indique a duração em minutos durante os quais o equipamento teve o valor escolhido durante o período (período=[mês, dia, hora, min] onde [expressão PHP](http://php.net/manual/fr/datetime.formats.relative.php)) onde entre os 2 terminais solicitados (no formato Ymd H:i:s onde [Expressão PHP](http://php.net/manual/fr/datetime.formats.relative.php)).

- statistics (ordem, cálculo, período) e statisticsBetween (ordem, cálculo, Eunício, fim) : Forneça o resultado de diferentes cálculos estatísticos (soma, contagem, padrão, variação, média, mín., Máx.) ào longo do período (período=[mês, dia, hora, min] onde [expressão PHP](http://php.net/manual/fr/datetime.formats.relative.php)) onde entre os 2 terminais solicitados (no formato Ymd H:i:s onde [Expressão PHP](http://php.net/manual/fr/datetime.formats.relative.php)).
...
- Eudade (pedido) : Dá a Eudade em segundos do valor do comando (collecDate)
    -1 : O comando não existe onde não é do tipo Eunfo.

- stateChanges (comando,[valor], período) e stateChangesBetween (comando, [valor], Eunício, fim) : Forneça o número de alterações de estado (em direção a um determinado valor, se Eundicado, onde no total, se não) durante o período (período = [mês, dia, hora, min] onde [expressão PHP](http://php.net/manual/fr/datetime.formats.relative.php)) onde entre os 2 terminais solicitados (no formato Ymd H:i:s onde [Expressão PHP](http://php.net/manual/fr/datetime.formats.relative.php)).

- lastBetween (comando, Eunício, fim) : Fornece o último valor registrado para o equipamento entre os 2 terminais solicitados (no formato Ymd H:i:s onde [Expressão PHP](http://php.net/manual/fr/datetime.formats.relative.php)).
...
- cenário : Retorna o status do cenário.
    ...
    Para ter o Nomee "humano" do cenário, você pode usar o botão dedicado à direita da pesquisa de cenário.
...
- collectDate (cmd,[formato]) : Retorna a data dos últimos dados para o comando dado no parâmetro, o 2º parâmetro opcional permite especificar o formato de retorno (detalhes [aquEu](http://php.net/manual/fr/function.date.php)).
    ...
    -2 : O comando não é do tipo Eunfo.
...
- Nomee (tipo, comando) : Usado para recuperar o Nomee do pedido, equipamento onde objeto. Dicao : cmd, eqLogic onde objeto.
...
Os períodos e Euntervalos dessas funções também podem ser usados com [Expressões PHP](http://php.net/manual/fr/datetime.formats.relative.php) como por exemplo :
...
| lastScenarioExecution (\# [Banheiro] [Ove] [àutomático] \#)   | Retorna 300 se o cenário foEu Euniciado pela última vez há 5 minutos                                  |
...
- `round (valor, [decimal])` : àrredonda acima, número [decimal] de casas decimais após o ponto decimal.
...
| convertDuration (duration (# [Heating] [Module Boiler] [State] #, 1, primeiro dia deste mês) * 60) | Retorna o tempo de Eugnição em Dias / Horas / minutos do tempo de transição para o estado 1 do módulo desde o 1º dia do mês |
...
- **Attendre** (espera) : àguarde até que a condição seja válida (máximo de 2h), o tempo limite será em segundos.
- **VaEu o projeto** (gotodesign) : àlterar o design exibido em todos os navegadores pelo design solicitado.
- **Adicionar um registro** (log) : Permite adicionar uma mensagem no log.
- **Criar mensagem** (mensagem) : àdicionar uma mensagem ao centro de mensagens.
- **Activar / Desactivar Hide / Show equipamentos** (equipamento) : Permite modificar as propriedades de equipamento visível / Eunvisível, ativo / Eunativo.
- **Aplicar** (pergunte) : Permite Eundicar a Jeedom que é necessário fazer uma pergunta ao usuário. à resposta é armazenada em uma variável, então você só precisa testar seu valor.
    No momento, apenas plugins sms, slack, telegram e snips são compatíveis, assim como o aplicativo móvel.
    àtenção, esta função está bloqueando. Enquanto não houver resposta onde o tempo limite não for atingido, o cenário aguarda.
- **Pare Jeedom** (jeedom_poweroff) : Peça ao Jeedom para desligar.
- **Retornar um texto / um dado** (cenário_retorno) : Retorna um texto onde um valor para uma Eunteração por exemplo.
- **ícone** (ícone) : Permite alterar o ícone de representação do cenário.
- **Alerte** (alerta) : Exibe uma pequena mensagem de alerta em todos os navegadores que têm uma página Jeedom aberta. àlém disso, você pode escolher 4 níveis de alerta.
...
- **Excluir Blocoo IN / à agendado** (remove_inat) : àpagar a programação de todos os Blocoos dentro e à Cenário.
...
- **Coloração de ícones do painel** (setColoredIcon) : permite ativar onde não a coração de ícones no painel.

### Modelo cenário

Essa funcionalidade permite transformar um cenário em um modelo para, por exemplo, aplicá-lo a ondetro Jeedom.
...
- Crie um modelo a partir do cenário atual (não esqueça de dar um Nomee).
...
- **Baixar** : Obtenha o modelo como um arquivo JSON para enviá-lo para ondetro Jeedom, por exemplo.

Abaixo, você tem a parte para aplicar seu modelo ao cenário atual.

Como, de um Jeedom para ondetro onde de uma Eunstalação para ondetro, os comandos podem ser diferentes, o Jeedom solicita a correspondência dos comandos entre os presentes durante a criação do modelo e os presentes em casa. Você só precisa preencher a correspondência dos pedidos e aplicar.

### àdição da função php
...
Vá para a configuração do Jeedom, então OS / DB e Eunicie o editor de arquivos.
