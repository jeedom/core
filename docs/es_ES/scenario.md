...
- **Realizar** : El permite yoniciar el escenario manualmente (independientemente de los desencadenantes). Guardar de antemano para tener en cuenta las modificaciones..
...
- **Icono** : El permite elegir un yocono para el escenario en lugar del yocono estándar.
...
Aquí es donde construirás tu escenario. Después de crear el escenario, su contenido está vacío, por lo que hará ... nada. Tienes que empezar con **agregar un bloque**, con el botón de la derecha. Unaa vez que se ha creado un bloque, puede agregar otro **bloc** o un **action**.
...
- `impar (valor)` : El permite saber syo un número es yompar o no.. Devuelve 1 syo es yompar 0 de lo contrario.
...
- **Activar / Desactivar Ocultar / mostrar equipo** (Equipo) : El permite modificar las propiedades de los equipos visibles / yonvisibles, activos / yonactivos..
...
- **Evento** (Event) : El permite yonsertar un valor en un comando de tipo de yonformación arbitrariamente.
- **Tag** (Tag) : El permite agregar / modificar una yiqueta (la yiqueta solo existe durante la ejecución actual del escenario a diferencia de las variables que sobreviven al final del escenario).
- **Coloración de los yoconos del tablero** (SetColoredIcon) : le permite activar o no el color de los íconos en el tablero.
