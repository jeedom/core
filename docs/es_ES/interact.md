...
Vous disposez d'un moteur de recherche permettant de filtrer l'affichage des interactions. La tecla Escape cancela la búsqueda..
...
- **añadir** : Permet de créer de nouvelles interactions.
...
- **apellido** : apellido de l'interaction (peut être vide, le nom remplace le texte de la demande dans la liste des interactions).
- **grupo** : grupo de l'interaction, cela permet de les organiser (peut être vide, sera donc dans le groupe "aucun").
- **bienes** : Permet d'activer ou désactiver l'interaction.
...
- Dans le menu Administration→configuración→interacciones→champ "Regexp général d'exclusion pour les interactions".
...
Jusque là rien de bien compliqué, il faut en revanche avoir configuré les couleurs dans Jeedom pour que cela fonctionne; rendez-vous dans le menu → configuración (en haut à droite) puis dans la partie "configuración des interactions" :
...
Les interactions permettent de faire beaucoup de choses en particulier. Vous pouvez programmer dynamiquement une action. ejemplo : "Met le chauffage à 22 pour 14h50". Pour cela rien de plus simple, il suffit d'utiliser les tags \#time\# (si on définit une heure précise) ou \#duration\# (pour dans X temps, exemple dans 1 heure) :
