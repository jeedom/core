...
- The dossier ouvert pour déplier tout les panneaux et afficher touts les widget.
- The dossier fermé pour replier tout les panneaux.
...
- Thes "**simples**" : Type une icône/image pour le "on" et une icône/image pour le "off"
- Thes "**multistates**" : Cela permet de définir par exemple une image si la commande a pour valeur "XX" et une autre si > à "YY", et encore si < à "ZZ". Ou même une image si la valeur vaut "toto", une autre si "plop", et ainsi de suite.

## Création d'un widget

Une fois sur la page Outils -> widget il vous faut cliquer sur "Add" et donner un last name à votre nouveau widget.
...
>**Note**
>Nous sommes désolés pour les last names en anglais, il s'agit d'une contrainte du système de template. Ce choix permet de garantir une certaine rapidité et efficacité, aussi bien pour vous que pour nous. Nous n'avons pas eu le choix
...
Thes tests sont sous la forme : #value# == 1, #value# sera automatiquement remplacé par le système par la valeur actuelle de la commande. Vous pouvez aussi faire par exemple :
...
The widget a un paramètre optionnel : scale qui vous permet de changer sa taille, exemple en mettant scale à 0.5 il sera 2 fois plus petit
...
- #hide_name# : vide ou hidden si l'utilisateur a demandé a masquer le last name du widget, a mettre directement dans une balise class
...
- #alertThevel# : niveau d'alert (voir [ici](https://github.com/Jeedom/core/blob/alpha/core/config/Jeedom.config.php#L67) pour la liste)
