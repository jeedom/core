...
Real brain of home automation, the scenarios allow to interact with the real world in a way **.
...
- **Overview** : Allows you to have an overview of all scenarios. You can change the values ****, ****, **multi launch**, **synchronous mode**, **** and **Timeline** (these parameters are described in the following chapter). You can also access the logs for each scenario and start them individually.
...
- **** : Displays the scenario logs.
- **** : Copy the scenario to create a new one with another name.
- **** : Allows you to view the graph of the elements related to the scenario.
...
>
>  **Ctrl Click on the execute button** allows you to directly save, execute and display the scenario log (if the log level is not None).
...
- **** : Allows you to organize the scenarios, by classifying them in groups (visible on the scenarios page and in their contextual menus).
- **Asset** : Activate the scenario. If not active, it will not be executed by Jeedom, regardless of the trigger mode.
- **** : Used to make the scenario visible (Dashboard).
...
>  : you can have a maximum of 28 triggers / programming for a scenario.
...
> **Tip**
>
> A Ctrl Shift Z or Ctrl Shift Y allows you to'**** or from **** a modification (addition of action, block...).
...
- On the left :
    ...
    - The eye reduces a block (**) to reduce its visual impact. Ctrl Click on the eye reduces them or displays them all.
...
-  ``cmd::byString($string);`` : Returns the corresponding command object.
    -   ``$string``: Link to the desired command : ``#[objet][][commande]#`` ( : ``#[Appartement][Alarme][Asset]#``)
...
-  **vertical double arrow** to move the action. Just drag and drop from there.
- A button for **** .
...
- ``not(…​ matches …​)`` : Does not contain.  :  ``not([Salle de bain][Hydrometrie][etat] matches "humide")``.
...
- ``#hour12#`` : Current time in 12-hour format (without leading zeros).  : 8 for 08:07:06.
- ``#minute#`` : Current minute (without leading zeros).  : 7 for 08:07:06.
- ``#day#`` : Current day (without leading zeros).  : 6 for 06/07/2017.
- ``#month#`` : Current month (without leading zeros).  : 7 for 06/07/2017.
- ``#year#`` : Current year.
- ``#time#`` : Current hour and minute.  : 1715 for 5.15 p.m.
- ``#timestamp#`` : Number of seconds since January 1, 1970.
- ``#date#`` : Day and month. Attention, the first number is the month.  : 1215 for December 15.
- ``#week#`` : Week number.
- ``#sday#`` : Name of day of week.  : .
- ``#nday#`` : Day number from 0 (Sunday) to 6 (Saturday).
- ``#smonth#`` : Name of the month.  : .
...
- ``lastBetween(commande,start,end)`` : Gives the last value recorded for the device between the 2 requested terminals (in the form ``Y-m-d H:i:s`` Where [PHP expression](http:php.netmanualfrdatetime.formats.relative.php)).
...
The periods and intervals of these functions can also be used with [PHP expressions](http:php.netmanualfrdatetime.formats.relative.php) For example :
...
- ``time_between(time,start,end)`` : Used to test if a time is between two values with ``time=temps`` ( : 1530), ``start=temps``, ``end=temps``. Start and end values can be straddling midnight.
...
- ``floor(time60)`` : Convert seconds to minutes, or minutes to hours (``floor(time3600)`` for seconds to hours).
...
| ``floor(13060)``                     | Returns 2 (minutes if 130s, or hours if 130m)                      |
...
- **** (sleep) : Pause of x second (s).
- **** (variable) : Creation / modification of a variable or the value of a variable.
- **Remove variable** () : Allows you to delete a variable.
...
- **Scenario** (scenario) : Allows you to control scenarios. The tags part allows you to send tags to the scenario, ex : montag = 2 (be careful, only use letters from a to z. No capital letters, no accents and no special characters). We recover the tag in the target scenario with the tag function (montag).
    ...
    -  : Activate a disabled scenario.
    - Deactivate : Disable scenario. It no longer launches regardless of the triggers.
    - Resetting the ISs : Used to reset the status of ****. This status is used for the non-repetition of the actions of a ****, if the evaluation of the condition gives the same result as the previous evaluation.
- **** (stop) : Stop the scenario.
- **** (wait) : Wait until the condition is valid (maximum 2h), the timeout is in seconds (s).
