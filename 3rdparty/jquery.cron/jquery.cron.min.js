(function (e) {
    var n = {initial: "* * * * *", minuteOpts: {minWidth: 100, itemWidth: 30, columns: 4, rows: undefined, title: "Minutes"}, timeHeureOpts: {minWidth: 100, itemWidth: 20, columns: 2, rows: undefined, title: "Duree: Heure"}, domOpts: {minWidth: 100, itemWidth: 30, columns: undefined, rows: 10, title: "Jour du mois"}, moisOpts: {minWidth: 100, itemWidth: 100, columns: 2, rows: undefined, title: undefined}, dowOpts: {minWidth: 100, itemWidth: undefined, columns: undefined, rows: undefined, title: undefined}, timeMinuteOpts: {minWidth: 100, itemWidth: 20, columns: 4, rows: undefined, title: "Duree: Minute"}, effectOpts: {openSpeed: 400, closeSpeed: 400, openEffect: "slide", closeEffect: "slide", hideOnMouseOut: true}, url_set: undefined, customValues: undefined, onChange: undefined, useGentleSelect: false};
    var y = "";
    for (var u = 0; u < 60; u++) {
        var t = (u < 10) ? "0" : "";
    ...
    var r = "";
    var b = ["minute", "heure", "jour", "semaine", "mois", "annee"];
    for (var u = 0; u < b.length; u++) {
        r += "<option value='" + b[u] + "'>" + b[u] + "</option>\n"
    }
    var p = {minute: [], heure: ["mins"], jour: ["time"], semaine: ["dow", "time"], mois: ["dom", "time"], annee: ["dom", "mois", "time"]};
    var w = {minute: /^(\*\s){4}\*$/, heure: /^\d{1,2}\s(\*\s){3}\*$/, jour: /^(\d{1,2}\s){2}(\*\s){2}\*$/, semaine: /^(\d{1,2}\s){2}(\*\s){2}\d{1,2}$/, mois: /^(\d{1,2}\s){3}\*\s\*$/, annee: /^(\d{1,2}\s){4}\*$/};
    function a(i) {
    ...
    function k(B) {
        ...
        switch (A) {
            ...
            case"jour":
                ...
                heure = i.time.find("select.cron-time-heure").val();
                break;
            case"semaine":
                j = i.time.find("select.cron-time-min").val();
                heure = i.time.find("select.cron-time-heure").val();
            ...
            case"mois":
                j = i.time.find("select.cron-time-min").val();
                heure = i.time.find("select.cron-time-heure").val();
                jour = i.dom.find("select").val();
                break;
            case"annee":
                j = i.time.find("select.cron-time-min").val();
                heure = i.time.find("select.cron-time-heure").val();
                jour = i.dom.find("select").val();
                mois = i.mois.find("select").val();
                break;
            default:
                return A
        }
        return[j, heure, jour, mois, dow].join(" ")
    }
    var x = {init: function (i) {
            var G = i ? i : {};
            var B = e.extend([], n, G);
            var j = e.extend({}, n.effectOpts, G.effectOpts);
            e.extend(B, {minuteOpts: e.extend({}, n.minuteOpts, j, G.minuteOpts), domOpts: e.extend({}, n.domOpts, j, G.domOpts), moisOpts: e.extend({}, n.moisOpts, j, G.moisOpts), dowOpts: e.extend({}, n.dowOpts, j, G.dowOpts), timeHeureOpts: e.extend({}, n.timeHeureOpts, j, G.timeHeureOpts), timeMinuteOpts: e.extend({}, n.timeMinuteOpts, j, G.timeMinuteOpts)});
            if (f(this, B)) {
                return this
            }
            ...
            C.period = e("<span class='cron-period'>Fréquence <select name='cron-period'>" + A + r + "</select> </span>").appendTo(this).data("root", this);
            var E = C.period.find("select");
            E.bind("change.cron", m.periodChanged).data("root", this);
            if (B.useGentleSelect) {
                E.gentleSelect(j)
            }
            C.dom = e("<span class='cron-block cron-block-dom'> le <select name='cron-dom'>" + v + "</select> </span>").appendTo(this).data("root", this);
            E = C.dom.find("select").data("root", this);
            if (B.useGentleSelect) {
                E.gentleSelect(B.domOpts)
            }
            C.mois = e("<span class='cron-block cron-block-mois'> de <select name='cron-mois'>" + h + "</select> </span>").appendTo(this).data("root", this);
            E = C.mois.find("select").data("root", this);
            if (B.useGentleSelect) {
                E.gentleSelect(B.moisOpts)
            }
            C.mins = e("<span class='cron-block cron-block-mins'> a <select name='cron-mins'>" + y + "</select> min</span>").appendTo(this).data("root", this);
            E = C.mins.find("select").data("root", this);
            if (B.useGentleSelect) {
                E.gentleSelect(B.minuteOpts)
            }
            C.dow = e("<span class='cron-block cron-block-dow'> le <select name='cron-dow'>" + s + "</select> </span>").appendTo(this).data("root", this);
            E = C.dow.find("select").data("root", this);
            if (B.useGentleSelect) {
                E.gentleSelect(B.dowOpts)
            }
            C.time = e("<span class='cron-block cron-block-time'> à <select name='cron-time-heure' class='cron-time-heure'>" + d + "</select>:<select name='cron-time-min' class='cron-time-min'>" + y + " </span>").appendTo(this).data("root", this);
            E = C.time.find("select.cron-time-heure").data("root", this);
            if (B.useGentleSelect) {
                E.gentleSelect(B.timeHeureOpts)
            }
            ...
            if (a(A.customValues) && A.customValues.hasOwnProperty(K)) {
                ...
                var H = C.split(" ");
                var J = {mins: H[0], heure: H[1], dom: H[2], mois: H[3], dow: H[4]};
                var G = p[K];
                for (var E = 0; E < G.length; E++) {
                    var B = G[E];
                    if (B == "time") {
                        var I = D[B].find("select.cron-time-heure").val(J.heure);
                        if (F) {
    ...
    var m = {periodChanged: function () {
            ...
            e.ajax({type: "POST", url: i.data("options").url_set, data: {cron: A}, success: function () {
