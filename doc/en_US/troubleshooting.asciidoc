...
If the scenario there is no pause it is advisable to check the "execute as quickly as possible" (available in expert mode)
...
Malheureusement, il n'y a pas beaucoup de solution si c'est le deuxième cas, le mieux étant de récupérer un backup (disponible dans /usr/share/nginx/www/jeedom/backup par défaut), de réinstaller Jeedom et de restaurer la sauvegarde. Vous pouvez aussi en SSH regarder pourquoi MySQL ne veut pas démarrer : 
[source,bash]
sudo su -
service mysql stop
mysqld --verbose
