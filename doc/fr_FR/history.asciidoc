...
A partir de là, vous devez avoir une fenetre qui apparait, il faut cliquer sur "Configuration avancée" : 
...
[TIP]
Il suffit de cliquer sur le nom de la pièce pour le déplier ; apparaissent les commandes historisées qui peuvent être graphées.

[TIP]
La couleur d'arrière-plan des objets de cette liste peuvent être modifiées à partir de la configuration de la pièce.
