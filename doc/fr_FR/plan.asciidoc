...
* *Ajouter zone* : Permet d'ajouter une zone transparente cliquable qui pourra exécuter une série d'action lors d'un clic (en fonction ou non de l'état d'une autre commande)
...
[NOTE]
L'élément séléctionné apparaît en surbrillance rouge (au lieu de vert pour tous les autres)
...
Permet de verrouiller l'élément pour qu'il ne soit plus déplacable ou redimmensionnable
...
[IMPORTANT]
Si vous mettez du code html (en particulier du javascript), attention à bien le vérifier avant car vous pouvez s'il y a une erreur dedans ou s'il écrase un composant Jeedom planter complètement le design et il ne restera plus qu'à le supprimer directement de la base de données
...
* *Caméra* : caméra à afficher (si vous avez choisi une caméra)
...
Dans ce mode Jeedom va exécuter la ou les action(s) On ou Off en fonction de l'état de la commande que vous donnez. Ex si la commande vaut 0 alors Jeedom exécutera la ou les action(s) On sinon il exécutera la ou les action(s) off
