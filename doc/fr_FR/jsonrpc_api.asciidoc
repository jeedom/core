...
- int id : id d'une commande ou tableau d'id si vous voulez exécuter plusieurs commandeé d'un coup
...
- string log : nom du log à recupérer
- string start : numéro de ligne sur laquelle commencer la lecture
- string nbLine : nombre de ligne à recupérer

=== log::list

Permet de récupérer la liste des logs de Jeedom

Paramètres:

- string filtre : (optionnel) filtre sur le nom des logs à recupérer
...
- string log : nom du log à supprimer
...
- string type : type de la valeur stockée (pour les scénarios c'est scenario (sans accent))
...
- int reply_cmd = NULL : ID de la commande à utiliser pour répondre, sinon préciser alors Jeedom vous renvoie la réponse dans le json
...
Liste des ports USB et des noms de clef USB branchés dessus
...
Renvoie les informations sur le statuts des dépendances du plugin
...
Renvoie les informations sur le statuts du démon du plugin
...
Retourne la liste de tous les composants installés, leur version et les informations associées
...
Renvoie le statuts du DNS Jeedom
