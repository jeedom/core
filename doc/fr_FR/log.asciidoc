...
Pour y accèder il faut aller dans Général -> Administration -> Log : 
...
Il existe certains paramètres de configuration des logs (accessible par Général -> Administation -> Configuration puis la partie "Configuration des logs & messages").
...
Le log "Event" est un peu particulier. Tout d'abord pour qu'il marche, il faut activer le niveau de log "Event", ensuite celui-ci recense tous les évènements ou actions qui se passent sur la domotique. Pour y accéder, il faut cliquer sur la petite icône "compteur" en bas à droite dans Jeedom : 
...
[IMPORTANT]
Pour que cela marche il faut absolument avoir activé le niveau de log "Event" (voir paragraphe précédent)
