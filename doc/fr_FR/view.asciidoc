...
Le principe est assez simple : une vue est composée de zones (on peut en mettre autant que l'on veut). Chaque zone est de type graphique ou widget : en fonction du type, vous pourrez y mettre des widgets ou des graphiques.
...
Au centre, vous avez un bouton pour renommer une vue, le bouton d'ajout de zone, un bouton pour voir le résultat, un pour sauvegarder (ne pas oublier de le faire avant de cliquer sur voir le résultat) et un pour supprimer la vue.
...
Une fois le bouton d'ajout/édition de widget pressé, vous obtenez : 
