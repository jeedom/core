...
On retrouve tout en haut un résumé du nombre de pièce existant, du nombre d'équipement et du nombre de commande. Sur la droite se trouve, un bouton pour masquer tous les équipements inactif et  un champs de recherche pour par exemple chercher un équipement,une commande ou tous les équipements d'un certain type (plugin, par exemple il suffit de taper rfxcom pour avoir tous les équipements rfxcom)

En dessous on retrouve un cadre par pièce, dans chaque cadre se trouve la liste des équipements (en bleu) contenu dans la pièce. Vous avez sur la ligne du nom de l'objets jusqu'a 3 boutons, dans l'ordre : 

* le premier sert à ouvrir la page de configuration de la pièce dans un nouvel onglet
* le deuxieme donne un résumé sur la pièce
* le dernier permet de masquer la liste des équipements (en dessous en bleu)

[TIP]
La couleur de fond des cadres pièce dépend de la couleur mise dans la configuration de la pièce

A partir de la vous pouvez en cliquant/maintenant un équipement changer l'ordre dans la pièce ou meme l'affecter à une autre pièce.
...
* *ID de la pièce* : identifiant unique de la pièce mère (peut etre vide)
...
* *Afficher le nom de la pièce* : uniquement sur les vues cela permet de ne pas afficher le nom de la pièce à coté du nom de l'équipement
