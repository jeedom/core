...
La fenêtre de configuration avancée d'un équipement permet de voir, de modifier et d'éditer la configuration de l'équipement

Vous avez en haut des boutons : 

* *Liens* : Permet d'afficher les liens de l'équipement avec les objets,commandes,scénario, variable, interaction.... sous forme graphique (dans un graphique un double clic sur un élément vous amène sur celui-ci)
* *Log* : affiche les évènements de l'équipement en question
* *Informations* : affiche les proprietés brutes de l'équipement
...
* *Arrondit des bordures (en px)* : permet d'arrondir les bordures, il faut mettre du code css, proprieté border-radius (ex "10px")
...
* Choisir la dimension du tableau ligne x colonne
...
* Ajouter un texte en plus de la commande (ou tout seul s'il n'y a pas de commande dans la case)
...
Il permet aussi de gérer le timeout de l'équipement, ex 30 indique à jeedom que si l'équipement n'a pas communiqué depuis plus de 30min alors il faut le mettre en alerte

=== Commentaire

Permet de mettre des commentaires sur l'équipement (date de change de pile par exemple)
...
[NOTE]
En fonction du type de la commande les informations/actions affichées peuvent changer
...
* *Action avant exécution de la commande* : action qui sera faite avant chaque exécution de la commande
* *Action après exécution de la commande* : action faite après chaque exécution de la commande

=== Alerte

Permet de définir un niveau d'alerte (warning ou danger) en fonction de certaines conditions, exemple si #value# > 8 pendant 30min alors l'équipement peut passer en alerte warning.

[NOTE]
Vous pouvez sur la page d'administration de Jeedom configurer une commande de type message qui permettra à Jeedom de vous prévenir si on atteint le seuil warning ou danger
