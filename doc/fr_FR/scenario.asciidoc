...
- lastChangeStateDuration(commande,valeur) : renvoi la durée en secondes depuis le dernier changement d'état à la valeur passée en paramètre.
    * Ex : lastChangeStateDuration(\#[Salle de bain][Hydrometrie][Humidité]#,0) : Renvoi 300 si cette valeur est passée à 0 la depuis 5 minutes (même si depuis sa valeur a changé).
