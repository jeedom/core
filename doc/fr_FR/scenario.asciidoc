...
* *Simple* : permet de faire des scénarios très simple type SI ça ALORS fait ça
...
* *Execution en avant plan* : permet de lancer le scénario directement et non dans un fil d'exécution séparé. Attention cette option peut être dangereuse car elle met en pause toutes les autres actions du scénario. IL NE FAUT JAMAIS l'activer sur un scénario déclenché par une programmation horaire ou un scénario contenant des actions de type sleep
* *Enchainer les commandes sans attendre* : permet d'enchainer les suites d'actions sans attendre le retour et donc la vérification de la bonne exécution (attention : il faut que le plugin le supporte, actuellement seuls les plugins openzwave et script sont compatibles)
...
[IMPORTANT]
Attention à bien mettre des ' ou " (quote ou double quote) lors de l'utilisation de chaînes de caractères. Ex : "\#[Object][Equipement][Info]#" = "oui"
...
* *\#time#* : heure et minute courante (ex : 1715 pour 17h15 ou 815 pour 08h15),
...
* *wait* : attend jusqu'à ce que la condition soit valide (maximum 2h), le timeout est en seconde                   
...
* *return* : retourne un message à la fin du scénario, ne sert que pour retourner un message spécifique suite a une intéraction. Attention à bien cocher la case "Exécuter le plus rapidement possible" pour que cela fonctionne
...
* *$scenario->setLog($message)*;
    ** Ecrit un message dans le log du scénario
...
Fonctionnalité permettant de transformer un scénario en template pour - par exemple - l'appliquer sur un autre Jeedom ou le partager sur le market. C'est aussi à partir de là que vous pouvez récupérer un scénario du market
...
* D'envoyer un template à Jeedom (fichier JSON préalablement recuperé)
...
* Consulter les templates actuellement présents sur votre Jeedom

Après avoir cliqué sur un template vous obtenez : 
