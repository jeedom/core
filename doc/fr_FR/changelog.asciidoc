...
* Correction d'orthographe (merci à dab0u pour son énorme travails)
* JEED-290 : On ne peut plus se connecter avec les identifiants par defaut (admin/admin) à distance, seul le réseaux local est autorisé
* JEED-186 : On peut maintenant choisir la couleur de fonc dans les designs
* Pour le bloc A possibilité de mettre une heure entre 00h01 et 00h59 en mettant simple les minutes (ex 30 pour 00h30)
* Ajout des sessions actives et des péripheriques enregistrés sur la page de profils de l'utilisateurs et la page de gestion des utilisateurs
* JEED-284 : la connexion permanant dépend maintenant d'une clef unique utilisateur et péripherique (et non plus que utilisateur)
...
* Optimisation des modifications d'organisation (taille des widgets, position des équipements, position des commandes) sur le dashboard et les vue. Attention maintenant les modification ne sont sauvegardées que lorsque l'on quitte le mode édition.
...
* Correction du bugs de déplacement des objets du désign sous firefox
...
* JEED-2 : Amélioration de la gestion de l'heure lors d'un redemarrage
